...
const instrumentation_enum
  request_list_constants::instrumentation_added[bucket_count] = {
...
const instrumentation_enum
  request_list_constants::instrumentation_moved[bucket_count] = {
...
const instrumentation_enum
  request_list_constants::instrumentation_removed[bucket_count] = {
...
const instrumentation_enum
  request_list_constants::instrumentation_total[bucket_count] = {
...
struct request_list_same_piece {
  request_list_same_piece(const Piece& p)
    : m_piece(p) {}

  bool operator()(const BlockTransfer* d) {
    return m_piece.index() == d->piece().index() &&
...
RequestList::~RequestList() {
  if (m_transfer != nullptr) {
    destruct_error("request dtor m_transfer != nullptr");
    return;
  }

  if (!m_queues.empty()) {
    destruct_error("request dtor m_queues not empty");
    return;
  }
...
RequestList::choked() {
  ...
  if (m_queues.queue_empty(bucket_queued) &&
      m_queues.queue_empty(bucket_unordered))
    return;
...
RequestList::delay_process_unordered() {
  m_last_unordered_position =
    std::min(m_last_unordered_position, unordered_size());

  instrumentation_update(INSTRUMENTATION_TRANSFER_REQUESTS_FINISHED,
                         m_last_unordered_position);
...
RequestList::downloading(const Piece& piece) {
  if (m_transfer != nullptr)
    throw internal_error(
      "RequestList::downloading(...) m_transfer != nullptr.");
  ...
  switch (itr.first) {
    ...
    case bucket_unordered:
      // Do unordered take of element here to avoid copy shifting the whole
      // deque.
      // (?)
      //
      // Alternatively, move back some elements to bucket_queued.

      if ((size_t)std::abs(std::distance(
            m_queues.begin(itr.first), itr.second)) < m_last_unordered_position)
        m_last_unordered_position--;
  ...
  if (piece.length() != m_transfer->piece().length()) {
    if (piece.length() != 0)
      throw communication_error(
        "Peer sent a piece with wrong, non-zero, length.");
...
RequestList::finished() {
  if (!is_downloading())
    throw internal_error(
      "RequestList::finished() called but no transfer is in progress.");

  if (!m_transfer->is_valid())
    throw internal_error(
      "RequestList::finished() called but transfer is invalid.");
...
RequestList::skipped() {
  if (!is_downloading())
    throw internal_error(
      "RequestList::skip() called but no transfer is in progress.");
...
RequestList::is_interested_in_active() const {
  for (auto transfer : *m_delegator->transfer_list())
    if (m_peerChunks->bitfield()->get(transfer->index()))
      return true;
