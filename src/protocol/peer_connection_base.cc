...
void
PeerConnectionBase::initialize(DownloadMain*      download,
                               PeerInfo*          peerInfo,
                               SocketFd           fd,
                               Bitfield*          bitfield,
  ...
  std::pair<ThrottleList*, ThrottleList*> throttles =
    m_download->throttles(m_peerInfo->socket_address());
  ...
  m_peerChunks.upload_throttle()->slot_activate() = std::bind(
    &SocketBase::receive_throttle_up_activate, static_cast<SocketBase*>(this));

  m_peerChunks.download_throttle()->set_list_iterator(
    m_down->throttle()->end());
  m_peerChunks.download_throttle()->slot_activate() =
    std::bind(&SocketBase::receive_throttle_down_activate,
              static_cast<SocketBase*>(this));
...
PeerConnectionBase::cleanup() {
  ...
  if (m_download == NULL)
    throw internal_error("PeerConnection::~PeerConnection() m_fd is valid but "
                         "m_state and/or m_net is NULL");
  ...
  m_download->info()->set_upload_unchoked(
    m_download->info()->upload_unchoked() - m_upChoke.unchoked());
  m_download->info()->set_download_unchoked(
    m_download->info()->download_unchoked() - m_downChoke.unchoked());
...
PeerConnectionBase::receive_upload_choke(bool choke) {
  if (choke == m_upChoke.choked())
    throw internal_error("PeerConnectionBase::receive_upload_choke(...) "
                         "already set to the same state.");
  ...
  if (choke) {
    m_download->info()->set_upload_unchoked(
      m_download->info()->upload_unchoked() - 1);
...
PeerConnectionBase::receive_download_choke(bool choke) {
  if (choke == m_downChoke.choked())
    throw internal_error("PeerConnectionBase::receive_download_choke(...) "
                         "already set to the same state.");
  ...
  if (choke) {
    m_download->info()->set_download_unchoked(
      m_download->info()->download_unchoked() - 1);
...
PeerConnectionBase::load_up_chunk() {
  if (m_upChunk.is_valid() && m_upChunk.index() == m_upPiece.index()) {
    ...
    //     m_upChunk.chunk()->preload(m_upPiece.offset(),
    //     m_upChunk.chunk()->size());

    if (lt_log_is_valid(LOG_INSTRUMENTATION_MINCORE))
      log_mincore_stats_func(
        m_upChunk.chunk()->is_incore(m_upPiece.offset(), m_upPiece.length()),
        false,
        m_incoreContinous);
...
PeerConnectionBase::cancel_transfer(BlockTransfer* transfer) {
  if (!get_fd().is_valid())
    throw internal_error(
      "PeerConnectionBase::cancel_transfer(...) !get_fd().is_valid()");

  if (transfer->peer_info() != peer_info())
    throw internal_error(
      "PeerConnectionBase::cancel_transfer(...) peer info doesn't match");
...
PeerConnectionBase::should_connection_unchoke(choke_queue* cq) const {
  if (cq == m_download->choke_group()->up_queue())
    return m_download->info()->upload_unchoked() <
           m_download->up_group_entry()->max_slots();

  if (cq == m_download->choke_group()->down_queue())
    return m_download->info()->download_unchoked() <
           m_download->down_group_entry()->max_slots();
...
PeerConnectionBase::down_chunk_start(const Piece& piece) {
  if (!request_list()->downloading(piece)) {
    if (piece.length() == 0) {
      LT_LOG_PIECE_EVENTS("(down) skipping_empty %" PRIu32 " %" PRIu32
                          " %" PRIu32,
                          piece.index(),
                          piece.offset(),
                          piece.length());
    } else {
      LT_LOG_PIECE_EVENTS("(down) skipping_unneeded %" PRIu32 " %" PRIu32
                          " %" PRIu32,
                          piece.index(),
                          piece.offset(),
                          piece.length());
  ...
  if (!m_downChunk.is_valid() || piece.index() != m_downChunk.index()) {
    down_chunk_release();
    m_downChunk =
      m_download->chunk_list()->get(piece.index(), ChunkList::get_writable);
...
PeerConnectionBase::down_chunk_finished() {
  if (!request_list()->transfer()->is_finished())
    throw internal_error(
      "PeerConnectionBase::down_chunk_finished() Transfer not finished.");
  ...
  LT_LOG_PIECE_EVENTS("(down) %s %" PRIu32 " %" PRIu32 " %" PRIu32,
                      ...
                      transfer->piece().index(),
                      transfer->piece().offset(),
                      transfer->piece().length());

  if (transfer->is_leader()) {
    if (!m_downChunk.is_valid())
      throw internal_error("PeerConnectionBase::down_chunk_finished() Transfer "
                           "is the leader, but no chunk allocated.");
  ...
  // empties too often.
  if (m_downChunk.is_valid() &&
      (request_list()->queued_empty() ||
       m_downChunk.index() != request_list()->next_queued_piece().index()))
    down_chunk_release();
...
PeerConnectionBase::down_chunk() {
  if (!m_down->throttle()->is_throttled(m_peerChunks.download_throttle()))
    throw internal_error("PeerConnectionBase::down_chunk() tried to read a "
                         "piece but is not in throttle list");

  if (!m_downChunk.chunk()->is_writable())
    throw internal_error("PeerConnectionBase::down_part() chunk not writable, "
                         "permission denided");

  uint32_t quota =
    m_down->throttle()->node_quota(m_peerChunks.download_throttle());
  ...
  ChunkIterator    itr(
    ...
    transfer->piece().offset() +
      std::min(transfer->position() + quota, transfer->piece().length()));
  ...
  m_down->throttle()->node_used(m_peerChunks.download_throttle(),
                                bytesTransfered);
...
PeerConnectionBase::down_chunk_from_buffer() {
  m_down->buffer()->consume(down_chunk_process(m_down->buffer()->position(),
                                               m_down->buffer()->remaining()));

  if (!m_request_list.transfer()->is_finished() &&
      m_down->buffer()->remaining() != 0)
    throw internal_error(
      "PeerConnectionBase::down_chunk_from_buffer() !transfer->is_finished() "
      "&& m_down->buffer()->remaining() != 0.");
...
PeerConnectionBase::down_chunk_skip() {
  ...
  if (!throttle->is_throttled(m_peerChunks.download_throttle()))
    throw internal_error("PeerConnectionBase::down_chunk_skip() tried to read "
                         "a piece but is not in throttle list");
  ...
  uint32_t length =
    read_stream_throws(m_nullBuffer,
                       std::min(quota,
  ...
  if (down_chunk_skip_process(m_nullBuffer, length) != length)
    throw internal_error(
      "PeerConnectionBase::down_chunk_skip() "
      "down_chunk_skip_process(m_nullBuffer, length) != length.");
...
PeerConnectionBase::down_chunk_skip_from_buffer() {
  m_down->buffer()->consume(down_chunk_skip_process(
    m_down->buffer()->position(), m_down->buffer()->remaining()));
...
PeerConnectionBase::down_chunk_process(const void* buffer, uint32_t length) {
  if (!m_downChunk.is_valid() ||
      m_downChunk.index() != m_request_list.transfer()->index())
    throw internal_error(
      "PeerConnectionBase::down_chunk_process(...) !m_downChunk.is_valid() "
      "|| m_downChunk.index() != m_request_list.transfer()->index().");
  ...
  m_downChunk.chunk()->from_buffer(
    buffer, transfer->piece().offset() + transfer->position(), length);
...
uint32_t
PeerConnectionBase::down_chunk_skip_process(const void* buffer,
                                            uint32_t    length) {
  BlockTransfer* transfer = m_request_list.transfer();
  ...
  if (transfer->position() > transfer->block()->leader()->position())
    throw internal_error("PeerConnectionBase::down_chunk_skip_process(...) "
                         "transfer is past the Block's position.");

  // If the transfer is valid, compare the downloaded data to the
  // leader.
  uint32_t compareLength = std::min(
    length, transfer->block()->leader()->position() - transfer->position());

  // The data doesn't match with what has previously been downloaded,
  // bork this transfer.
  if (!m_downChunk.chunk()->compare_buffer(buffer,
                                           transfer->piece().offset() +
                                             transfer->position(),
                                           compareLength)) {
    LT_LOG_PIECE_EVENTS("(down) download_data_mismatch %" PRIu32 " %" PRIu32
                        " %" PRIu32,
                        transfer->piece().index(),
                        transfer->piece().offset(),
                        transfer->piece().length());
  ...
  if (down_chunk_process(static_cast<const char*>(buffer) + compareLength,
                         length - compareLength) != length - compareLength)
    throw internal_error("PeerConnectionBase::down_chunk_skip_process(...) "
                         "down_chunk_process(...) returned wrong value.");
...
PeerConnectionBase::down_extension() {
  if (m_down->buffer()->remaining()) {
    uint32_t need = std::min(m_extensions->read_need(),
                             (uint32_t)m_down->buffer()->remaining());
    std::memcpy(
      m_extensions->read_position(), m_down->buffer()->position(), need);
  ...
  if (!m_extensions->is_complete()) {
    uint32_t bytes = read_stream_throws(m_extensions->read_position(),
                                        m_extensions->read_need());
...
PeerConnectionBase::up_chunk_encrypt(uint32_t quota) {
  if (m_encryptBuffer == NULL)
    throw internal_error(
      "PeerConnectionBase::up_chunk: m_encryptBuffer is NULL.");
  ...
  if (m_encryptBuffer->remaining() == 0) {
    ...
    quota = std::min<uint32_t>(quota - m_encryptBuffer->remaining(),
                               m_encryptBuffer->reserved_left());
  }

  m_upChunk.chunk()->to_buffer(m_encryptBuffer->end(),
                               m_upPiece.offset() +
                                 m_encryptBuffer->remaining(),
                               quota);
...
PeerConnectionBase::up_chunk() {
  if (!m_up->throttle()->is_throttled(m_peerChunks.upload_throttle()))
    throw internal_error("PeerConnectionBase::up_chunk() tried to write a "
                         "piece but is not in throttle list");

  if (!m_upChunk.chunk()->is_readable())
    throw internal_error(
      "ProtocolChunk::write_part() chunk not readable, permission denided");
  ...
  if (is_encrypted()) {
    ...
    Chunk::data_type data;
    ChunkIterator    itr(m_upChunk.chunk(),
                      m_upPiece.offset(),
                      m_upPiece.offset() + std::min(quota, m_upPiece.length()));
...
PeerConnectionBase::up_extension() {
  if (m_extensionOffset == extension_must_encrypt) {
    if (m_extensionMessage.owned()) {
      m_encryption.encrypt(m_extensionMessage.data(),
                           m_extensionMessage.length());

    } else {
      char* buffer = new char[m_extensionMessage.length()];

      m_encryption.encrypt(
        m_extensionMessage.data(), buffer, m_extensionMessage.length());
      m_extensionMessage.set(
        buffer, buffer + m_extensionMessage.length(), true);
    }
  ...
  uint32_t written =
    write_stream_throws(m_extensionMessage.data() + m_extensionOffset,
                        m_extensionMessage.length() - m_extensionOffset);
  ...
  if (m_extensions->is_complete() && !m_extensions->is_invalid()) {
    ...
    if (!m_extensions->read_done())
      throw internal_error("PeerConnectionBase::up_extension could not process "
                           "complete extension message.");
...
PeerConnectionBase::read_request_piece(const Piece& p) {
  ...
  if (m_upChoke.choked() || itr != m_peerChunks.upload_queue()->end() ||
      p.length() > (1 << 17)) {
    LT_LOG_PIECE_EVENTS("(up)   request_ignored  %" PRIu32 " %" PRIu32
                        " %" PRIu32,
                        p.index(),
                        p.offset(),
                        p.length());
  ...
  LT_LOG_PIECE_EVENTS("(up)   request_added    %" PRIu32 " %" PRIu32
                      " %" PRIu32,
                      p.index(),
                      p.offset(),
                      p.length());
...
PeerConnectionBase::read_cancel_piece(const Piece& p) {
  ...
  if (itr != m_peerChunks.upload_queue()->end()) {
    ...
    LT_LOG_PIECE_EVENTS("(up)   cancel_requested %" PRIu32 " %" PRIu32
                        " %" PRIu32,
                        p.index(),
                        p.offset(),
                        p.length());
  } else {
    LT_LOG_PIECE_EVENTS("(up)   cancel_ignored   %" PRIu32 " %" PRIu32
                        " %" PRIu32,
                        p.index(),
                        p.offset(),
                        p.length());
...
PeerConnectionBase::write_prepare_piece() {
  ...
  if (!m_download->file_list()->is_valid_piece(m_upPiece) ||
    ...
    snprintf(buffer,
             128,
             "Peer requested an invalid piece: %u %u %u",
             m_upPiece.index(),
             m_upPiece.length(),
             m_upPiece.offset());

    LT_LOG_PIECE_EVENTS("(up)   invalid_piece_in_upload_queue %" PRIu32
                        " %" PRIu32 " %" PRIu32,
                        m_upPiece.index(),
                        m_upPiece.length(),
                        m_upPiece.offset());
  ...
  LT_LOG_PIECE_EVENTS("(up)   prepared         %" PRIu32 " %" PRIu32
                      " %" PRIu32,
                      m_upPiece.index(),
                      m_upPiece.length(),
                      m_upPiece.offset());
...
void
PeerConnectionBase::write_prepare_extension(int               type,
...
PeerConnectionBase::should_request() {
  if (m_downChoke.choked() || !m_downInterested || !m_downUnchoked)
    ...
    return m_downStall <= 1 ||
           m_download->info()->down_rate()->rate() < (10 << 10);
}
...
PeerConnectionBase::try_request_pieces() {
  ...
  uint32_t pipeSize = request_list()->calculate_pipe_size(
    m_peerChunks.download_throttle()->rate()->rate());
  ...
  while (request_list()->queued_size() < pipeSize &&
         m_up->can_write_request()) {
    ...
    if (!m_download->file_list()->is_valid_piece(*p) ||
        !m_peerChunks.bitfield()->get(p->index()))
      throw internal_error("PeerConnectionBase::try_request_pieces() tried to "
                           "use an invalid piece.");
...
PeerConnectionBase::send_pex_message() {
  ...
  if (m_sendPEXMask & (PEX_ENABLE | PEX_DISABLE)) {
    if (!m_extensions->is_remote_supported(ProtocolExtension::UT_PEX))
      throw internal_error(
        "PeerConnectionBase::send_pex_message() Not supported by peer.");

    write_prepare_extension(
      ProtocolExtension::HANDSHAKE,
      ProtocolExtension::generate_toggle_message(
        ProtocolExtension::UT_PEX, (m_sendPEXMask & PEX_ENABLE) != 0));

    m_sendPEXMask &= ~(PEX_ENABLE | PEX_DISABLE);

  } else if (m_sendPEXMask & PEX_DO &&
             m_extensions->id(ProtocolExtension::UT_PEX)) {
    const DataBuffer& pexMessage =
      m_download->get_ut_pex(m_extensions->is_initial_pex());
...
PeerConnectionBase::send_ext_message() {
  write_prepare_extension(m_extensions->pending_message_type(),
                          m_extensions->pending_message_data());
