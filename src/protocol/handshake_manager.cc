...
ProtocolExtension HandshakeManager::DefaultExtensions =
...
HandshakeManager::erase(Handshake* handshake) {
  ...
  if (itr == base_type::end())
    throw internal_error(
      "HandshakeManager::erase(...) could not find handshake.");
...
HandshakeManager::add_outgoing(const utils::socket_address& sa,
  ...
  create_outgoing(
    sa, download, manager->connection_manager()->encryption_options());
}
...
HandshakeManager::create_outgoing(const utils::socket_address& sa,
  ...
  if (!fd.open_stream() || !setup_socket(fd) ||
  ...
  else if (encryptionOptions & (ConnectionManager::encryption_try_outgoing |
  ...
  LT_LOG_SA(&sa,
            "Adding outcoming connection: encryption:%x message:%x.",
            encryptionOptions,
            message);
  manager->connection_manager()->inc_socket_count();
...
HandshakeManager::receive_succeeded(Handshake* handshake) {
  if (!handshake->is_active())
    throw internal_error("HandshakeManager::receive_succeeded(...) called on "
                         "an inactive handshake.");
  ...
  if (download->info()->is_active() &&
      download->connection_list()->want_connection(handshake->peer_info(),
                                                   handshake->bitfield()) &&

      (pcb = download->connection_list()->insert(
         handshake->peer_info(),
         handshake->get_fd(),
         handshake->bitfield(),
    ...
    manager->client_list()->retrieve_id(
      ...
      handshake->peer_info()->id());
    LT_LOG_SA_C(
      handshake->peer_info()->socket_address(), "Handshake success.", 0);

    pcb->peer_chunks()->set_have_timer(handshake->initialized_time());

    if (handshake->unread_size() != 0) {
      if (handshake->unread_size() >
          PeerConnectionBase::ProtocolRead::buffer_size)
        throw internal_error("HandshakeManager::receive_succeeded(...) Unread "
                             "data won't fit PCB's read buffer.");
    ...
    else if (download->file_list()->is_done() &&
             handshake->bitfield()->is_all_set())
...
HandshakeManager::receive_failed(Handshake* handshake, int message, int error) {
  ...
  if (handshake->encryption()->should_retry()) {
    int retry_options =
      handshake->retry_options() | ConnectionManager::encryption_retrying;
    DownloadMain* download = handshake->download();
...
HandshakeManager::setup_socket(SocketFd fd) {
  ...
  if (m->send_buffer_size() != 0 &&
      !fd.set_send_buffer_size(m->send_buffer_size()))
    return false;

  if (m->receive_buffer_size() != 0 &&
      !fd.set_receive_buffer_size(m->receive_buffer_size()))
    return false;
