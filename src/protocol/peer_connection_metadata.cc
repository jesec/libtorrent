...
PeerConnectionMetadata::read_message() {
  ...
  switch (buf->read_8()) {
    ...
    case ProtocolBase::PIECE:
      throw communication_error(
        "Received a piece but the connection is strictly for meta data.");
    ...
    do {
      switch (m_down->get_state()) {
        ...
        case ProtocolRead::READ_EXTENSION:
          ...
          // Drop peer if it disabled the metadata extension.
          if (!m_extensions->is_remote_supported(
                ProtocolExtension::UT_METADATA))
            throw close_connection();
...
PeerConnectionMetadata::read_skip_bitfield() {
  if (m_down->buffer()->remaining()) {
    uint32_t length =
      std::min(m_skipLength, (uint32_t)m_down->buffer()->remaining());
...
PeerConnectionMetadata::try_request_metadata_pieces() {
  if (m_download->file_list()->chunk_size() == 1 ||
      !m_extensions->is_remote_supported(ProtocolExtension::UT_METADATA))
    return false;
  ...
  // DEBUG:
  //   if (!request_list()->queued_size() < pipeSize ||
  ...
  if (!m_download->file_list()->is_valid_piece(*p) ||
    ...
    throw internal_error("PeerConnectionMetadata::try_request_metadata_pieces()"
                         " tried to use an invalid piece.");
...
PeerConnectionMetadata::receive_metadata_piece(uint32_t    piece,
  ...
  if (data == NULL) {
    ...
    if ((piece << ProtocolExtension::metadata_piece_shift) +
          ProtocolExtension::metadata_piece_size >=
        m_download->file_list()->size_bytes())
      length = m_download->file_list()->chunk_size() %
               ProtocolExtension::metadata_piece_size;

    m_tryRequest = false;
    read_cancel_piece(
      Piece(0, piece << ProtocolExtension::metadata_piece_shift, length));
