...
HandshakeEncryption::should_retry() const {
  return (m_options & ConnectionManager::encryption_enable_retry) != 0 &&
         m_retry != HandshakeEncryption::RETRY_NONE;
}

bool
HandshakeEncryption::initialize() {
  m_key = new DiffieHellman(
    dh_prime, dh_prime_length, dh_generator, dh_generator_length);
...
HandshakeEncryption::initialize_decrypt(const char* origHash, bool incoming) {
  ...
  sha1_salt(
    incoming ? "keyA" : "keyB", 4, m_key->c_str(), 96, origHash, 20, hash);
...
HandshakeEncryption::initialize_encrypt(const char* origHash, bool incoming) {
  ...
  sha1_salt(
    incoming ? "keyB" : "keyA", 4, m_key->c_str(), 96, origHash, 20, hash);
...
HandshakeEncryption::hash_req1_to_sync() {
  sha1_salt("req1", 4, m_key->c_str(), m_key->size(), modify_sync(20));
}
