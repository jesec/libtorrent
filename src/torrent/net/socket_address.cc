...
sin6_addr32_index(const sockaddr_in6* sa, unsigned int index) {
  return (sa->sin6_addr.s6_addr[index * 4 + 0] << 24) +
...
inline in6_addr
sin6_make_addr32(uint32_t addr0,
                 uint32_t addr1,
                 uint32_t addr2,
                 uint32_t addr3) {
  uint32_t addr32[4];
  ...
  switch (sa->sa_family) {
    ...
    case AF_INET6:
      if (sa_is_v4mapped(sa))
        return sin6_addr32_index(reinterpret_cast<const sockaddr_in6*>(sa),
                                 3) == htonl(INADDR_ANY);
  ...
  switch (sa->sa_family) {
    ...
    default:
      throw internal_error(
        "torrent::sa_convert: sockaddr is not a valid family");
  }
}
...
sa_copy_addr(const sockaddr* sa, uint16_t port) {
  ...
  switch (sa->sa_family) {
    ...
    default:
      throw internal_error(
        "torrent::sa_copy_addr: sockaddr is not a valid family");
  }
}
...
sa_copy_addr_in6(const sockaddr_in6* sa, uint16_t port) {
  ...
  std::memcpy(&reinterpret_cast<sockaddr_in6*>(result.get())->sin6_addr,
              &sa->sin6_addr,
              sizeof(in6_addr));
...
sa_from_v4mapped(const sockaddr* sa) {
  ...
  return sa_from_in(
    sin_from_v4mapped_in6(reinterpret_cast<const sockaddr_in6*>(sa)));
}
...
sa_to_v4mapped(const sockaddr* sa) {
  ...
  return sa_from_in6(
    sin6_to_v4mapped_in(reinterpret_cast<const sockaddr_in*>(sa)));
}
...
sin_from_v4mapped_in6(const sockaddr_in6* sin6) {
  if (!sin6_is_v4mapped(sin6))
    throw internal_error(
      "torrent::sin6_is_v4mapped: sockaddr_in6 is not v4mapped");

  sin_unique_ptr result = sin_make();
  result.get()->sin_addr.s_addr =
    reinterpret_cast<in_addr_t>(htonl(sin6_addr32_index(sin6, 3)));
...
sin6_to_v4mapped_in(const sockaddr_in* sin) {
  ...
  result.get()->sin6_addr =
    sin6_make_addr32(0, 0, 0xffff, ntohl(sin->sin_addr.s_addr));
...
sin_from_sa(sa_unique_ptr&& sap) {
  if (!sap_is_inet(sap))
    throw internal_error(
      "torrent::sin_from_sa: sockaddr is nullptr or not inet");
...
sin6_from_sa(sa_unique_ptr&& sap) {
  if (!sap_is_inet6(sap))
    throw internal_error(
      "torrent::sin6_from_sa: sockaddr is nullptr or not inet6");
...
sin_from_c_sa(c_sa_unique_ptr&& sap) {
  if (!sap_is_inet(sap))
    throw internal_error(
      "torrent::sin_from_c_sa: sockaddr is nullptr or not inet");
...
sin6_from_c_sa(sa_unique_ptr&& sap) {
  if (!sap_is_inet6(sap))
    throw internal_error(
      "torrent::sin6_from_c_sa: sockaddr is nullptr or not inet6");

  return c_sin6_unique_ptr(
    reinterpret_cast<const sockaddr_in6*>(sap.release()));
}
...
sa_equal(const sockaddr* lhs, const sockaddr* rhs) {
  switch (rhs->sa_family) {
    ...
    default:
      throw internal_error(
        "torrent::sa_equal: rhs sockaddr is not a valid family");
  }

  switch (lhs->sa_family) {
    case AF_INET:
      return lhs->sa_family == rhs->sa_family &&
             sin_equal(reinterpret_cast<const sockaddr_in*>(lhs),
                       reinterpret_cast<const sockaddr_in*>(rhs));
    case AF_INET6:
      return lhs->sa_family == rhs->sa_family &&
             sin6_equal(reinterpret_cast<const sockaddr_in6*>(lhs),
                        reinterpret_cast<const sockaddr_in6*>(rhs));
    case AF_UNSPEC:
      return lhs->sa_family == rhs->sa_family;
    default:
      throw internal_error(
        "torrent::sa_equal: lhs sockaddr is not a valid family");
  }
}
...
sin6_equal(const sockaddr_in6* lhs, const sockaddr_in6* rhs) {
  return lhs->sin6_port == rhs->sin6_port &&
         std::equal(lhs->sin6_addr.s6_addr,
                    lhs->sin6_addr.s6_addr + 16,
...
sa_equal_addr(const sockaddr* lhs, const sockaddr* rhs) {
  switch (rhs->sa_family) {
    ...
    default:
      throw internal_error(
        "torrent::sa_equal_addr: rhs sockaddr is not a valid family");
  }

  switch (lhs->sa_family) {
    case AF_INET:
      return lhs->sa_family == rhs->sa_family &&
             sin_equal_addr(reinterpret_cast<const sockaddr_in*>(lhs),
                            reinterpret_cast<const sockaddr_in*>(rhs));
    case AF_INET6:
      return lhs->sa_family == rhs->sa_family &&
             sin6_equal_addr(reinterpret_cast<const sockaddr_in6*>(lhs),
                             reinterpret_cast<const sockaddr_in6*>(rhs));
    case AF_UNSPEC:
      return lhs->sa_family == rhs->sa_family;
    default:
      throw internal_error(
        "torrent::sa_equal_addr: lhs sockaddr is not a valid family");
  }
}
...
sin6_equal_addr(const sockaddr_in6* lhs, const sockaddr_in6* rhs) {
  return std::equal(lhs->sin6_addr.s6_addr,
                    lhs->sin6_addr.s6_addr + 16,
                    rhs->sin6_addr.s6_addr);
}
