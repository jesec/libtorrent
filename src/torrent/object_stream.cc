    ...
    case 'l':
      ...
      *object = Object::create_list();
      ...
      while (input->good()) {
        ...
        Object::list_iterator itr =
          object->as_list().insert(object->as_list().end(), Object());
...
object_read_bencode_skip_c(const char* first, const char* last) {
  ...
  while (first != last) {
    ...
    // string.
    if (*stack_itr &&
        (first = object_read_bencode_c_string(first, last).end()) == last)
      break;
...
const char*
object_read_bencode_raw_c(const char*      first,
                          const char*      last,
...
object_sha1(const Object* object) {
  ...
  sha.init();
  object_write_bencode_c(&object_write_to_sha1,
                         &sha,
                         object_buffer_t(buffer, buffer + 1024),
                         object);
  sha.final_c(buffer);
...
void
object_write_bencode_c_string(object_write_data_t* output,
                              const char*          srcData,
                              uint32_t             srcLength) {
  while (srcLength != 0) {
    uint32_t len = std::min<uint32_t>(
      srcLength, std::distance(output->pos, output->buffer.second));
...
object_write_bencode_c_value(object_write_data_t* output, int64_t src) {
  ...
  object_write_bencode_c_string(
    output, first, 20 - std::distance(buffer, first));
}
...
inline void
object_write_bencode_c_obj_string(object_write_data_t* output,
                                  const char*          data,
                                  uint32_t             size) {
...
void
object_write_bencode_c_object(object_write_data_t* output,
                              const Object*        object,
                              uint32_t             skip_mask) {
  switch (object->type()) {
    ...
    case Object::TYPE_VALUE:
      object_write_bencode_c_obj_value(output, object->as_value());
      break;
    case Object::TYPE_STRING:
      object_write_bencode_c_obj_string(output, object->as_string());
      break;
...
object_write_bencode(std::ostream* output,
                     ...
                     uint32_t      skip_mask) {
  char buffer[1024];
  object_write_bencode_c(&object_write_to_stream,
                         output,
                         object_buffer_t(buffer, buffer + 1024),
                         object,
                         skip_mask);
...
object_write_bencode(char*         first,
  ...
  return object_write_bencode_c(
    &object_write_to_buffer, &buffer, buffer, object, skip_mask);
}
...
object_write_bencode_c(object_write_t  writeFunc,
  ...
  return output.writeFunc(output.data,
                          object_buffer_t(output.buffer.first, output.pos));
}
...
object_write_to_sha1(void* data, object_buffer_t buffer) {
  reinterpret_cast<Sha1*>(data)->update(
    buffer.first, std::distance(buffer.first, buffer.second));
...
object_write_to_stream(void* data, object_buffer_t buffer) {
  reinterpret_cast<std::ostream*>(data)->write(
    buffer.first, std::distance(buffer.first, buffer.second));
...
object_write_to_size(void* data, object_buffer_t buffer) {
  *reinterpret_cast<uint64_t*>(data) +=
    std::distance(buffer.first, buffer.second);
...
static_map_read_bencode_c(const char*                    first,
  ...
  //       std::distance(first, last), (unsigned int)*first, escaped.c_str());
  ...
  while (first != last) {
    ...
    // to find a match.
    if (raw_key.size() >=
        static_map_mapping_type::max_key_size - stack_itr->next_key) {
      first = object_read_bencode_skip_c(first, last);
      continue;
    }
    ...
    // position...
    static_map_key_search_result key_search =
      find_key_match(first_key, last_key, current_key);
    ...
    switch (key_search.first->key[key_search.second]) {
      case '\0':
        first = object_read_bencode_c(
          first, last, &entry_values[key_search.first->index].object);
        first_key = key_search.first + 1;
        break;

      case '*':
        first = object_read_bencode_raw_c(
          first,
          last,
      ...
      case ':':
        ...
        stack_itr++;
        stack_itr->set_key_index(
          (stack_itr - 1)->next_key, key_search.second, 2);
      ...
      case '[': {
        ...
        while (first != last) {
          ...
          if (first_key->key[key_search.second + 2] == '*') {
            ...
            first = object_read_bencode_c(
              first, last, &entry_values[first_key->index].object);
          }

          if (++first_key == last_key ||
              strcmp(first_key->key, (first_key - 1)->key) != 0) {
            while (first != last) {
              if (*first == 'e') {
                first++;
                break;
      ...
      default:
        throw internal_error(
          "static_map_read_bencode_c: key_search.first->key[base] returned "
          "invalid character.");
    };
  }
  ...
  while (first_key != last_key) {
    ...
    do {
      ...
      if (stack_itr->obj_type == Object::TYPE_MAP)
        object_write_bencode_c_obj_string(
          output, key_begin, std::distance(key_begin, key_end));

      // Check if '::' or '[' were found...
      if (*key_end == ':' && *(key_end + 1) == ':') {
        (++stack_itr)
          ->set_key_index(std::distance(first_key->key, key_begin),
                          std::distance(first_key->key, key_end),
                          2);
      ...
      object_write_bencode_c_object(
        output, &entry_values[first_key->index].object, 0);
...
static_map_write_bencode_c_wrap(object_write_t                 writeFunc,
  ...
  //       //char buffer[1024];
  //       //      sprintf(buffer, "Verified wrong, %u, '%u', '%s'.",
  //       std::distanescaped.c_str());

  //       throw torrent::internal_error("Invalid bencode data generated: '" +
  //       escaped + "'");
  //     }
  //   }
