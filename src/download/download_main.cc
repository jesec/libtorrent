...
DownloadInfo::DownloadInfo()
  : m_flags(flag_compact | flag_accepting_new_peers | flag_accepting_seeders |
            flag_pex_enabled | flag_pex_active)
  ,
...
DownloadMain::DownloadMain()
  ...
  m_tracker_list->slot_success() =
    std::bind(&TrackerController::receive_success,
              m_tracker_controller,
              std::placeholders::_1,
              std::placeholders::_2);
  m_tracker_list->slot_failure() =
    std::bind(&TrackerController::receive_failure,
              m_tracker_controller,
              std::placeholders::_1,
              std::placeholders::_2);
  m_tracker_list->slot_scrape_success() =
    std::bind(&TrackerController::receive_scrape,
              m_tracker_controller,
              std::placeholders::_1);
  m_tracker_list->slot_tracker_enabled() =
    std::bind(&TrackerController::receive_tracker_enabled,
              m_tracker_controller,
              std::placeholders::_1);
  m_tracker_list->slot_tracker_disabled() =
    std::bind(&TrackerController::receive_tracker_disabled,
              m_tracker_controller,
              std::placeholders::_1);

  m_connectionList = new ConnectionList(this);

  m_delegator.slot_chunk_find() = std::bind(&ChunkSelector::find,
                                            m_chunkSelector,
                                            std::placeholders::_1,
                                            std::placeholders::_2);
  m_delegator.slot_chunk_size() =
    std::bind(&FileList::chunk_index_size, file_list(), std::placeholders::_1);

  m_delegator.transfer_list()->slot_canceled() = std::bind(
    &ChunkSelector::not_using_index, m_chunkSelector, std::placeholders::_1);
  m_delegator.transfer_list()->slot_queued() = std::bind(
    &ChunkSelector::using_index, m_chunkSelector, std::placeholders::_1);
  m_delegator.transfer_list()->slot_completed() =
    std::bind(&DownloadMain::receive_chunk_done, this, std::placeholders::_1);
  m_delegator.transfer_list()->slot_corrupt() = std::bind(
    &DownloadMain::receive_corrupt_chunk, this, std::placeholders::_1);

  m_delayDisconnectPeers.slot() =
    std::bind(&ConnectionList::disconnect_queued, m_connectionList);
  m_taskTrackerRequest.slot() =
    std::bind(&DownloadMain::receive_tracker_request, this);

  m_chunkList->set_data(file_list()->mutable_data());
  m_chunkList->slot_create_chunk() = std::bind(&FileList::create_chunk_index,
                                               file_list(),
                                               std::placeholders::_1,
                                               std::placeholders::_2);
  m_chunkList->slot_free_diskspace() =
    std::bind(&FileList::free_diskspace, file_list());
}
...
DownloadMain::~DownloadMain() {
  ...
  if (m_info->size_pex() != 0) {
    destruct_error("DownloadMain::~DownloadMain(): m_info->size_pex() != 0.");
    return;
  }
...
DownloadMain::stop() {
  ...
  connection_list()->erase_remaining(connection_list()->begin(),
  ...
  if (info()->upload_unchoked() != 0 || info()->download_unchoked() != 0)
    throw internal_error("DownloadMain::stop(): info()->upload_unchoked() != 0 "
                         "|| info()->download_unchoked() != 0.");
}
...
DownloadMain::initial_seeding_done(PeerConnectionBase* pcb) {
  ...
  if (pcb_itr != m_connectionList->end()) {
    ...
    m_connectionList->erase_remaining(m_connectionList->begin() + 1,
                                      ConnectionList::disconnect_available);
  } else {
    m_connectionList->erase_remaining(m_connectionList->begin(),
                                      ConnectionList::disconnect_available);
  }
...
DownloadMain::update_endgame() {
  if (!m_delegator.get_aggressive() && file_list()->completed_chunks() +
                                           m_delegator.transfer_list()->size() +
                                           5 >=
                                         file_list()->size_chunks())
...
DownloadMain::receive_chunk_done(unsigned int index) {
  ...
  if (!handle.is_valid())
    throw storage_error("DownloadState::chunk_done(...) called with an index "
                        "we couldn't retrieve from storage");
...
DownloadMain::receive_tracker_request() {
  if ((info()->is_pex_enabled() && info()->size_pex()) > 0 ||
      connection_list()->size() + peer_list()->available_list()->size() / 2 >=
        connection_list()->min_size()) {
...
struct SocketAddressCompact_less {
  bool operator()(const SocketAddressCompact& a,
                  const SocketAddressCompact& b) const {
...
DownloadMain::do_peer_exchange() {
  if (!info()->is_active())
    throw internal_error(
      "DownloadMain::do_peer_exchange called on inactive download.");
  ...
  if (!m_info->is_pex_active() &&
      ...
      m_peerList.available_list()->size() <
        m_peerList.available_list()->max_size() / 4) {
    ...
    //              m_peerList.available_list()->size() >=
    //              m_peerList.available_list()->max_size() / 2) {
  ...
  for (ConnectionList::iterator itr = m_connectionList->begin();
    ...
    } else if (!pcb->extensions()->is_local_enabled(
                 ProtocolExtension::UT_PEX)) {
      continue;
    ...
    if (!message->empty() && (message->data() == m_ut_pex_initial.data() ||
                              message->data() == m_ut_pex_delta.data())) {
  ...
  if (current.size() > m_info->max_size_pex_list()) {
    ...
    if (added.size() < current.size() - m_info->max_size_pex_list())
      throw internal_error("DownloadMain::do_peer_exchange() added.size() < "
                           "current.size() - m_info->max_size_pex_list().");

    // Randomize this:
    added.erase(added.end() - (current.size() - m_info->max_size_pex_list()),
                added.end());

    // Create the new m_ut_pex_list by removing any 'removed'
    // addresses from the original list and then adding the new
    // addresses.
    m_ut_pex_list.erase(std::set_difference(m_ut_pex_list.begin(),
                                            m_ut_pex_list.end(),
                                            removed.begin(),
                                            removed.end(),
                                            m_ut_pex_list.begin(),
    ...
    std::sort(
      m_ut_pex_list.begin(), m_ut_pex_list.end(), SocketAddressCompact_less());
  ...
  // the delta message stays emptied. Otherwise generate the appropriate
  // messages.
  if (!added.empty() || !m_ut_pex_list.empty()) {
    ...
    m_ut_pex_initial.clear();
    m_ut_pex_initial =
      ProtocolExtension::generate_ut_pex_message(m_ut_pex_list, current);
  }
}
