...
DownloadWrapper::DownloadWrapper()
  ...
  m_main->delay_download_done().slot() =
    std::bind(&download_data::call_download_done, data());
  m_main->delay_partially_done().slot() =
    std::bind(&download_data::call_partially_done, data());
  m_main->delay_partially_restarted().slot() =
    std::bind(&download_data::call_partially_restarted, data());

  m_main->peer_list()->set_info(info());
  m_main->tracker_list()->set_info(info());
  m_main->tracker_controller()->slot_success() = std::bind(
    &DownloadWrapper::receive_tracker_success, this, std::placeholders::_1);
  m_main->tracker_controller()->slot_failure() = std::bind(
    &DownloadWrapper::receive_tracker_failed, this, std::placeholders::_1);

  m_main->chunk_list()->slot_storage_error() = std::bind(
    &DownloadWrapper::receive_storage_error, this, std::placeholders::_1);
}
...
DownloadWrapper::initialize(const std::string& hash, const std::string& id) {
  ...
  info()->slot_completed() =
    std::bind(&FileList::completed_bytes, m_main->file_list());
  ...
  m_hashChecker->slot_check_chunk() =
    std::bind(&DownloadWrapper::check_chunk_hash, this, std::placeholders::_1);
  m_hashChecker->delay_checked().slot() =
    std::bind(&DownloadWrapper::receive_initial_hash, this);
}
...
DownloadWrapper::close() {
  ...
  m_main->chunk_list()->sync_chunks(
    ChunkList::sync_all | ChunkList::sync_force | ChunkList::sync_sloppy |
    ChunkList::sync_ignore_error);
...
DownloadWrapper::is_stopped() const {
  return !m_main->tracker_controller()->is_active() &&
         !m_main->tracker_list()->has_active();
}
...
DownloadWrapper::receive_initial_hash() {
  if (info()->is_active())
    throw internal_error(
      "DownloadWrapper::receive_initial_hash() but we're in a bad state.");
  ...
  if (!m_hashChecker->is_checking()) {
    ...
    if (hash_queue()->has(data()))
      throw internal_error("DownloadWrapper::receive_initial_hash() found a "
                           "chunk in the HashQueue.");
...
DownloadWrapper::receive_hash_done(ChunkHandle handle, const char* hash) {
  if (!handle.is_valid())
    throw internal_error(
      "DownloadWrapper::receive_hash_done(...) called on an invalid chunk.");

  if (!info()->is_open())
    throw internal_error("DownloadWrapper::receive_hash_done(...) called but "
  ...
  if (hash != NULL) {
    ...
    if (data()->untouched_bitfield()->get(handle.index()))
      throw internal_error("DownloadWrapper::receive_hash_done(...) received a "
                           "chunk that isn't set in ChunkSelector.");

    if (std::memcmp(hash, chunk_hash(handle.index()), 20) == 0) {
      bool was_partial = data()->wanted_chunks() != 0;

      m_main->file_list()->mark_completed(handle.index());
      m_main->delegator()->transfer_list()->hash_succeeded(handle.index(),
                                                           handle.chunk());
      ...
      if (!m_main->have_queue()->empty() &&
          m_main->have_queue()->front().first >= cachedTime)
        m_main->have_queue()->push_front(
          DownloadMain::have_queue_type::value_type(
            m_main->have_queue()->front().first + 1, handle.index()));
      else
        m_main->have_queue()->push_front(
          DownloadMain::have_queue_type::value_type(cachedTime,
                                                    handle.index()));

    } else {
      // This needs to ensure the chunk is still valid.
      m_main->delegator()->transfer_list()->hash_failed(handle.index(),
                                                        handle.chunk());
    }
  }
...
DownloadWrapper::check_chunk_hash(ChunkHandle handle) {
  ...
  ChunkHandle new_handle =
    m_main->chunk_list()->get(handle.index(), ChunkList::get_blocking);
  m_main->chunk_list()->release(&handle);

  hash_queue()->push_back(new_handle,
                          data(),
                          std::bind(&DownloadWrapper::receive_hash_done,
                                    this,
                                    std::placeholders::_1,
                                    std::placeholders::_2));
}
  ...
  if (ticks % 4 == 0) {
    ...
    for (ConnectionList::iterator itr = m_main->connection_list()->begin();
      ...
      if (!(*itr)->m_ptr()->receive_keepalive())
        itr = m_main->connection_list()->erase(
          itr, ConnectionList::disconnect_available);
      else
        itr++;
  }
...
DownloadWrapper::finished_download() {
  ...
  if (!m_main->delay_download_done().is_queued())
    priority_queue_insert(
      &taskScheduler, &m_main->delay_download_done(), cachedTime);
