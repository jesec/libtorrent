...
DownloadWrapper::DownloadWrapper()
  ...
  m_main->delay_partially_restarted().slot() = [this]() {
    data()->call_partially_restarted();
  };

  m_main->peer_list()->set_info(info());
  m_main->tracker_list()->set_info(info());

  m_main->tracker_controller()->slot_success() =
    [this](torrent::AddressList* l) { return receive_tracker_success(l); };

  m_main->tracker_controller()->slot_failure() =
    [this](const std::string& msg) { receive_tracker_failed(msg); };

  m_main->chunk_list()->slot_storage_error() = [this](const std::string& msg) {
    receive_storage_error(msg);
  };
}
...
DownloadWrapper::initialize(const std::string& hash, const std::string& id) {
  ...
  info()->slot_completed() = [this]() {
    return m_main->file_list()->completed_bytes();
  };
  ...
  m_hashChecker->slot_check_chunk() = [this](ChunkHandle ch) {
    check_chunk_hash(ch);
  };
...
DownloadWrapper::check_chunk_hash(ChunkHandle handle) {
  ...
  hash_queue()->push_back(
    new_handle, data(), [this](ChunkHandle handle, const char* hash) {
      receive_hash_done(handle, hash);
    });
}
...
DownloadWrapper::receive_tick(uint32_t ticks) {
  ...
  if (ticks % 4 == 0) {
    ...
    for (auto itr = m_main->connection_list()->begin();
