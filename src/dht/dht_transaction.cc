...
DhtSearch::~DhtSearch() {
  ...
  // case.
  if (m_pending) {
    destruct_error("DhtSearch::~DhtSearch called with pending transactions.");
    return;
  }

  if (m_concurrency != 3) {
    destruct_error("DhtSearch::~DhtSearch with invalid concurrency limit.");
    return;
  }
...
DhtSearch::add_contact(const HashString& id, const utils::socket_address* sa) {
  auto n     = new DhtNode(HashString(id.data()), sa);
...
DhtSearch::add_contacts(const DhtBucket& contacts) {
  ...
  // Add max_contacts=18 closest nodes, and fill up so we also have at least 8
  // good nodes.
  int needClosest = max_contacts - size();
  int needGood    = DhtBucket::num_nodes;

  for (auto itr = chain.bucket()->begin(); needClosest > 0 || needGood > 0;
       ++itr) {
    ...
    if ((!(*itr)->is_bad() || needClosest > 0) &&
        add_contact((*itr)->id(), (*itr)->address())) {
      needGood -= !(*itr)->is_bad();
      needClosest--;
    }
  }
...
// currently being contacted, nor has it been found to be good or bad.
bool
...
DhtSearch::node_status(const_accessor& n, bool success) {
  if (n == end() || !n.node()->is_active())
    throw internal_error(
      "DhtSearch::node_status called for invalid/inactive node.");
...
DhtAnnounce::start_announce() {
  ...
  if (!complete() || m_next != end() || size() > DhtBucket::num_nodes)
    throw internal_error(
      "DhtSearch::start_announce called in inconsistent state.");
...
DhtTransactionPacket::build_buffer(const DhtMessage& msg) {
  char buffer[1500]; // If the message would exceed an Ethernet frame, something
                     // went very wrong.
...
DhtTransactionSearch::set_stalled() {
  if (!m_hasQuickTimeout)
    throw internal_error("DhtTransactionSearch::set_stalled called on already "
                         "stalled transaction.");
...
DhtTransactionSearch::complete(bool success) {
  if (m_node == m_search->end())
    throw internal_error(
      "DhtTransactionSearch::complete called multiple times.");

  if (m_node.search() != m_search)
    throw internal_error(
      "DhtTransactionSearch::complete called for node from wrong search.");
