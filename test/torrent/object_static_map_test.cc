...
#include "test/torrent/object_static_map_test.h"
...
const test_map_type::key_list_type test_map_type::keys = {
  { key_d_a, "d_a::b" }, { key_d_d_a, "d_a::c::a" }, { key_d_b, "d_a::d" },

  { key_e_0, "e[]" },    { key_e_1, "e[]" },         { key_e_2, "e[]" },

  { key_s_a, "s_a" },    { key_s_b, "s_b" },

  { key_v_a, "v_a" },    { key_v_b, "v_b" },
};

template<>
const test_map_2_type::key_list_type test_map_2_type::keys = {
  { key_2_d_x_y, "d_x::f" }, { key_2_d_x_z, "d_x::g" },
  { key_2_l_x_1, "l_x[]" },  { key_2_l_x_2, "l_x[]" },
  { key_2_s_a, "s_a" },      { key_2_v_a, "v_a" },
};

TEST_F(ObjectStaticMapTest, test_basics) {
...
TEST_F(ObjectStaticMapTest, test_write) {
  ...
  //   std::cout << "static map write: '" << std::string(buffer,
  //   std::distance(buffer, result.first)) << "'" << std::endl;

  ASSERT_TRUE(validate_bencode(buffer, result.first));
}

static const char* test_read_bencode =
  "d3:d_xd1:fi3e1:gli4ei5eee3:l_xli1ei2ei3ee3:s_a1:a3:v_ai2ee";
static const char* test_read_skip_bencode =
  "d3:d_xd1:fi3e1:gli4ei5eee1:fi1e3:l_xli1ei2ei3ee3:s_a1:a3:v_ai2ee";

template<typename map_type>
bool
static_map_read_bencode(map_type& map, const char* str) {
  try {
    return torrent::static_map_read_bencode(str, str + strlen(str), map) ==
           str + strlen(str);
  } catch (torrent::bencode_error&) {
    return false;
  }
}
...
template<typename map_type>
bool
static_map_read_bencode_exception(map_type& map, const char* str) {
  try {
...
TEST_F(ObjectStaticMapTest, test_read) {
  ...
  ASSERT_TRUE(static_map_read_bencode(test_skip_map, test_read_skip_bencode));
}
...
const ext_test_message::key_list_type ext_test_message::keys = {
  { key_e, "e" }, { key_m_utPex, "m::ut_pex" },
  { key_p, "p" }, { key_reqq, "reqq" },
  { key_v, "v" },
};

TEST_F(ObjectStaticMapTest, test_read_extensions) {
  ext_test_message test_ext;

  ASSERT_TRUE(static_map_read_bencode(test_ext, "d1:ai1ee"));

  ASSERT_TRUE(static_map_read_bencode(test_ext, "d1:mi1ee"));
  ASSERT_TRUE(static_map_read_bencode(test_ext, "d1:mdee"));
  ASSERT_TRUE(static_map_read_bencode(test_ext, "d6:ut_pexi0ee"));
  ASSERT_TRUE(static_map_read_bencode(test_ext, "d1:md6:ut_pexi0eee"));
}

template<typename map_type>
bool
static_map_write_bencode(map_type map, const char* original) {
  try {
    char  buffer[1023];
...
typedef torrent::static_map_type<keys_raw_types, key_raw_types_LAST>
  test_raw_types_type;
typedef torrent::static_map_type<keys_multiple, key_multiple_LAST>
                                                           test_multiple_type;
...
template<>
const test_single_type::key_list_type test_single_type::keys = { { key_single_a,
                                                                   "b" } };
template<>
const test_raw_type::key_list_type test_raw_type::keys = { { key_raw_a,
                                                             "b*" } };
template<>
const test_raw_types_type::key_list_type test_raw_types_type::keys = {
  { key_raw_types_empty, "e*" },
  { key_raw_types_list, "l*L" },
  { key_raw_types_map, "m*M" },
  { key_raw_types_str, "s*S" }
};
template<>
const test_multiple_type::key_list_type test_multiple_type::keys = {
  { key_multiple_a, "a" },
  { key_multiple_b, "b*" },
  { key_multiple_c, "c" }
};
template<>
const test_dict_type::key_list_type test_dict_type::keys = { { key_dict_a_b,
                                                               "a::b" } };

TEST_F(ObjectStaticMapTest, test_read_empty) {
  test_empty_type map_normal;
  test_empty_type map_skip;
  test_empty_type map_incomplete;

  ASSERT_TRUE(static_map_read_bencode(map_normal, "de"));
  ASSERT_TRUE(static_map_read_bencode(map_skip, "d1:ai1ee"));

  ASSERT_TRUE(static_map_read_bencode_exception(map_incomplete, ""));
  ASSERT_TRUE(static_map_read_bencode_exception(map_incomplete, "d"));
}

TEST_F(ObjectStaticMapTest, test_read_single) {
  ...
  ASSERT_TRUE(static_map_read_bencode_exception(map_incomplete, "d1:bi1"));
  ASSERT_TRUE(static_map_read_bencode_exception(map_incomplete, "d1:bi1e"));
}

TEST_F(ObjectStaticMapTest, test_read_single_raw) {
  test_raw_type map_raw;

  ASSERT_TRUE(static_map_read_bencode(map_raw, "d1:bi1ee"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_a].as_raw_bencode(), "i1e"));

  ASSERT_TRUE(static_map_read_bencode(map_raw, "d1:b2:abe"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_a].as_raw_bencode(), "2:ab"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_a].as_raw_bencode().as_raw_string(), "ab"));

  ASSERT_TRUE(static_map_read_bencode(map_raw, "d1:bli1ei2eee"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_a].as_raw_bencode(), "li1ei2ee"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_a].as_raw_bencode().as_raw_list(), "i1ei2e"));

  ASSERT_TRUE(static_map_read_bencode(map_raw, "d1:bd1:ai1e1:bi2eee"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_a].as_raw_bencode(), "d1:ai1e1:bi2ee"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_a].as_raw_bencode().as_raw_map(), "1:ai1e1:bi2e"));
}

TEST_F(ObjectStaticMapTest, test_read_raw_types) {
  test_raw_types_type map_raw;

  ASSERT_TRUE(static_map_read_bencode(map_raw,
                                      "d"
                                      "1:ei1e"
                                      "1:lli1ei2ee"
                                      "1:md1:ai1e1:bi2ee"
                                      ...
                                      "e"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_types_empty].as_raw_bencode(), "i1e"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_types_str].as_raw_string(), "ab"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_types_list].as_raw_list(), "i1ei2e"));
  ASSERT_TRUE(torrent::raw_bencode_equal_c_str(
    map_raw[key_raw_types_map].as_raw_map(), "1:ai1e1:bi2e"));
}

TEST_F(ObjectStaticMapTest, test_read_multiple) {
...
TEST_F(ObjectStaticMapTest, test_read_dict) {
  ...
  ASSERT_TRUE(static_map_read_bencode(map_normal, "d1:adee"));
  ASSERT_TRUE(map_normal[key_dict_a_b].is_empty());
...
TEST_F(ObjectStaticMapTest, test_write_empty) {
  ...
  ASSERT_TRUE(static_map_write_bencode(map_normal, "de"));
}

TEST_F(ObjectStaticMapTest, test_write_single) {
  test_single_type map_value;

  ASSERT_TRUE(static_map_write_bencode(map_value, "de"));

  map_value[key_single_a] = (int64_t)1;
  ASSERT_TRUE(static_map_write_bencode(map_value, "d1:bi1ee"));

  map_value[key_single_a] = "test";
  ASSERT_TRUE(static_map_write_bencode(map_value, "d1:b4:teste"));

  map_value[key_single_a] = torrent::Object::create_list();
  map_value[key_single_a].as_list().push_back("test");
  ASSERT_TRUE(static_map_write_bencode(map_value, "d1:bl4:testee"));
  ...
  ASSERT_TRUE(static_map_write_bencode(map_value, "d1:bl1:a2:bbee"));

  map_value[key_single_a] = torrent::raw_map("1:a2:bb", strlen("1:a2:bb"));
  ASSERT_TRUE(static_map_write_bencode(map_value, "d1:bd1:a2:bbee"));
}

TEST_F(ObjectStaticMapTest, test_write_multiple) {
