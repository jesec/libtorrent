...
test_output(const char* output, unsigned int length, unsigned int mask) {
  CPPUNIT_ASSERT_MESSAGE("'" + std::string(output) + "' != '" +
...
test_log::test_basic() {
  ...
  CPPUNIT_ASSERT(std::find_if(torrent::log_groups.begin(),
                              torrent::log_groups.end(),
                              std::bind(&torrent::log_group::valid,
                                        std::placeholders::_1)) ==
                 torrent::log_groups.end());
}
...
test_log::test_output_open() {
  ...
  // CPPUNIT_ASSERT_THROW(torrent::log_open_output("test_output_1",
...
test_log::test_file_output() {
  ...
  CPPUNIT_ASSERT_MESSAGE(
    buffer, std::string(buffer).find("test_file") != std::string::npos);
}
...
test_log::test_file_output_append() {
  ...
  CPPUNIT_ASSERT_MESSAGE(buffer_line1,
                         std::string(buffer_line1).find("test_line_1") !=
                           std::string::npos);
  CPPUNIT_ASSERT_MESSAGE(buffer_line2,
                         std::string(buffer_line2).find("test_line_2") !=
                           std::string::npos);
}
