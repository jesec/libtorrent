...
static bool
verify_did_internal_error(std::function<unsigned int()> func,
                          bool                          should_throw) {
...
test_signal_bitfield::test_basic() {
  ...
  CPPUNIT_ASSERT(torrent::signal_bitfield::max_size ==
  ...
  for (unsigned int i = 0; i < torrent::signal_bitfield::max_size; i++)
    CPPUNIT_ASSERT(signal_bitfield.add_signal(
...
test_signal_bitfield::test_multiple() {
  ...
  signal_bitfield.work();
  CPPUNIT_ASSERT(marked_bitfield ==
...
test_signal_bitfield::test_threaded() {
  ...
  for (unsigned int i = 0; i < torrent::signal_bitfield::max_size; i++)
    CPPUNIT_ASSERT(thread->signal_bitfield()->add_signal(
                     std::bind(&mark_index, &marked_bitfield, i)) == i);
