...
TEST_F(tracker_list_features_test, test_new_peers) {
  ...
  ASSERT_EQ(tracker_0->latest_sum_peers(), 20);
...
TEST_F(tracker_list_features_test, test_find_next_to_request) {
  ...
  ASSERT_EQ(tracker_list.find_next_to_request(tracker_list.begin()),
            tracker_list.begin());
  ASSERT_EQ(tracker_list.find_next_to_request(tracker_list.begin() + 1),
            tracker_list.begin() + 1);
  ASSERT_EQ(tracker_list.find_next_to_request(tracker_list.end()),
            tracker_list.end());
...
TEST_F(tracker_list_features_test, test_count_active) {
  ...
  tracker_list.send_state_idx(0, 1);
  ASSERT_EQ(tracker_list.count_active(), 1);

  tracker_list.send_state_idx(3, 1);
  ASSERT_EQ(tracker_list.count_active(), 2);

  tracker_list.send_state_idx(1, 1);
  tracker_list.send_state_idx(2, 1);
  ASSERT_EQ(tracker_list.count_active(), 4);
...
TEST_F(tracker_list_features_test, test_request_safeguard) {
  ...
  for (unsigned int i = 0; i < 9; i++) {
    ...
    ASSERT_EQ(tracker_1->success_counter(), (i + 1));
  }
  ...
  for (unsigned int i = 0; i < 9; i++) {
    ...
    ASSERT_EQ(tracker_foo->success_counter(), (i + 1));
  ...
  for (unsigned int i = 0; i < 40; i++) {
    ...
    ASSERT_EQ(tracker_2->success_counter(), (i + 1));
  ...
  for (unsigned int i = 0; i < 17; i++) {
    ...
    ASSERT_EQ(tracker_3->success_counter(), (i + 1));
