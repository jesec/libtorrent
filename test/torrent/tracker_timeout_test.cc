...
TEST_F(tracker_timeout_test, test_set_timeout) {
  ...
  ASSERT_TRUE(tracker.normal_interval() == 1800);

  tracker.set_new_normal_interval(100);
  ASSERT_TRUE(tracker.normal_interval() == 600);
  tracker.set_new_normal_interval(8 * 4000);
  ASSERT_TRUE(tracker.normal_interval() == 8 * 3600);

  tracker.set_new_min_interval(100);
  ASSERT_TRUE(tracker.min_interval() == 300);
  tracker.set_new_min_interval(4 * 4000);
  ASSERT_TRUE(tracker.min_interval() == 4 * 3600);
}
...
TEST_F(tracker_timeout_test, test_timeout_tracker) {
  ...
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 0);
  ...
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == ~uint32_t());
  ...
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 1800);
  tracker.send_state(torrent::Tracker::EVENT_NONE);
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == ~uint32_t());
  tracker.send_state(torrent::Tracker::EVENT_SCRAPE);
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 1800);
  ...
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 1800 - 3);
  tracker.set_success(1, torrent::cachedTime.seconds() + 3);
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 1800 + 3);
...
TEST_F(tracker_timeout_test, test_timeout_requesting) {
  ...
  tracker.close();
  tracker.set_failed(1, torrent::cachedTime.seconds());
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 5);
  tracker.set_failed(2, torrent::cachedTime.seconds());
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 10);
  tracker.set_failed(6 + 1, torrent::cachedTime.seconds());
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 320);
  ...
  // ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 10);
  // tracker.set_success(1, torrent::cachedTime.seconds());
  // ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 20);
  // tracker.set_success(2, torrent::cachedTime.seconds());
  ASSERT_TRUE(torrent::tracker_next_timeout(&tracker, flags) == 600);
