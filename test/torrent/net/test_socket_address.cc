...
TEST_F(test_socket_address, test_sa_is_any) {
  ...
  ASSERT_TRUE(!torrent::sap_is_any(sin_bc));
  ASSERT_TRUE(!torrent::sap_is_any(sin_1));
  ASSERT_TRUE(!torrent::sap_is_any(sin6_1));
  ASSERT_TRUE(!torrent::sap_is_any(sin_bc_5000));
  ASSERT_TRUE(!torrent::sap_is_any(sin_1_5000));
  ASSERT_TRUE(!torrent::sap_is_any(sin6_1_5000));

  ASSERT_TRUE(!torrent::sap_is_any(c_sin_bc));
  ASSERT_TRUE(!torrent::sap_is_any(c_sin_1));
  ASSERT_TRUE(!torrent::sap_is_any(c_sin6_1));
  ASSERT_TRUE(!torrent::sap_is_any(c_sin_bc_5000));
  ASSERT_TRUE(!torrent::sap_is_any(c_sin_1_5000));
  ASSERT_TRUE(!torrent::sap_is_any(c_sin6_1_5000));
}
...
TEST_F(test_socket_address, test_sa_is_broadcast) {
  ...
  ASSERT_TRUE(!torrent::sap_is_broadcast(sin_any));
  ASSERT_TRUE(!torrent::sap_is_broadcast(sin_1));
  ASSERT_TRUE(!torrent::sap_is_broadcast(sin6_any));
  ASSERT_TRUE(!torrent::sap_is_broadcast(sin6_1));
  ASSERT_TRUE(!torrent::sap_is_broadcast(sin_any_5000));
  ASSERT_TRUE(!torrent::sap_is_broadcast(sin_1_5000));
  ASSERT_TRUE(!torrent::sap_is_broadcast(sin6_any_5000));
  ASSERT_TRUE(!torrent::sap_is_broadcast(sin6_1_5000));

  ASSERT_TRUE(!torrent::sap_is_broadcast(c_sin_any));
  ASSERT_TRUE(!torrent::sap_is_broadcast(c_sin_1));
  ASSERT_TRUE(!torrent::sap_is_broadcast(c_sin6_any));
  ASSERT_TRUE(!torrent::sap_is_broadcast(c_sin6_1));
  ASSERT_TRUE(!torrent::sap_is_broadcast(c_sin_any_5000));
  ASSERT_TRUE(!torrent::sap_is_broadcast(c_sin_1_5000));
  ASSERT_TRUE(!torrent::sap_is_broadcast(c_sin6_any_5000));
  ASSERT_TRUE(!torrent::sap_is_broadcast(c_sin6_1_5000));
}

TEST_F(test_socket_address, test_make) {
  torrent::sa_unique_ptr sa_unspec = torrent::sa_make_unspec();
  ASSERT_TRUE(sa_unspec != nullptr);
  ASSERT_TRUE(sa_unspec->sa_family == AF_UNSPEC);

  torrent::sa_unique_ptr sa_inet = torrent::sa_make_inet();
  ASSERT_TRUE(sa_inet != nullptr);
  ASSERT_TRUE(sa_inet->sa_family == AF_INET);

  sockaddr_in* sin_inet = reinterpret_cast<sockaddr_in*>(sa_inet.get());
  ASSERT_TRUE(sin_inet->sin_family == AF_INET);
  ASSERT_TRUE(sin_inet->sin_port == 0);
  ASSERT_TRUE(sin_inet->sin_addr.s_addr == in_addr().s_addr);

  torrent::sa_unique_ptr sa_inet6 = torrent::sa_make_inet6();
  ASSERT_TRUE(sa_inet6 != nullptr);
  ASSERT_TRUE(sa_inet6->sa_family == AF_INET6);

  sockaddr_in6* sin6_inet6 = reinterpret_cast<sockaddr_in6*>(sa_inet6.get());
  ASSERT_TRUE(sin6_inet6->sin6_family == AF_INET6);
  ASSERT_TRUE(sin6_inet6->sin6_port == 0);
  ASSERT_TRUE(sin6_inet6->sin6_flowinfo == 0);
  ASSERT_TRUE(compare_sin6_addr(sin6_inet6->sin6_addr, in6_addr{ { { 0 } } }));
  ASSERT_TRUE(sin6_inet6->sin6_scope_id == 0);

  torrent::sa_unique_ptr sa_unix = torrent::sa_make_unix("");
  ASSERT_TRUE(sa_unix != nullptr);
  ASSERT_TRUE(sa_unix->sa_family == AF_UNIX);
}
...
TEST_F(test_socket_address, test_sin_from_sa) {
  ...
  ASSERT_TRUE(sa_zero != nullptr);
  ASSERT_NO_THROW({ sin_zero = torrent::sin_from_sa(std::move(sa_zero)); });
  ASSERT_TRUE(sa_zero == nullptr);
  ASSERT_TRUE(sin_zero != nullptr);

  ASSERT_TRUE(sin_zero->sin_addr.s_addr == htonl(0x0));
  ...
  ASSERT_TRUE(sa_inet == nullptr);
  ASSERT_TRUE(sin_inet != nullptr);

  ASSERT_TRUE(sin_inet->sin_addr.s_addr == htonl(0x01020304));
...
TEST_F(test_socket_address, test_sin6_from_sa) {
  ...
  ASSERT_TRUE(sa_zero == nullptr);
  ASSERT_TRUE(sin6_zero != nullptr);

  ASSERT_TRUE(sin6_zero->sin6_addr.s6_addr[0] == 0x0);
  ASSERT_TRUE(sin6_zero->sin6_addr.s6_addr[1] == 0x0);
  ASSERT_TRUE(sin6_zero->sin6_addr.s6_addr[15] == 0x0);
  ...
  ASSERT_TRUE(sa_inet6 == nullptr);
  ASSERT_TRUE(sin6_inet6 != nullptr);

  ASSERT_TRUE(sin6_inet6->sin6_addr.s6_addr[0] == 0xff);
  ASSERT_TRUE(sin6_inet6->sin6_addr.s6_addr[1] == 0x01);
  ASSERT_TRUE(sin6_inet6->sin6_addr.s6_addr[15] == 0x01);
...
TEST_F(test_socket_address, test_sa_equal) {
  ...
  ASSERT_TRUE(!torrent::sap_equal(sin_1, sin_2));
  ASSERT_TRUE(!torrent::sap_equal(sin_1, c_sin_2));
  ASSERT_TRUE(!torrent::sap_equal(c_sin_1, sin_2));
  ASSERT_TRUE(!torrent::sap_equal(c_sin_1, c_sin_2));
  ...
  ASSERT_TRUE(!torrent::sap_equal(sin6_1, sin6_2));
  ASSERT_TRUE(!torrent::sap_equal(sin6_1, c_sin6_2));
  ASSERT_TRUE(!torrent::sap_equal(c_sin6_1, sin6_2));
  ASSERT_TRUE(!torrent::sap_equal(c_sin6_1, c_sin6_2));

  ASSERT_TRUE(torrent::sap_equal(sin_1_5000, sin_1_5000));
  ASSERT_TRUE(torrent::sap_equal(sin6_1_5000, sin6_1_5000));
  ASSERT_TRUE(!torrent::sap_equal(sin_1_5000, sin_1_5100));
  ASSERT_TRUE(!torrent::sap_equal(sin6_1_5000, sin6_1_5100));
  ASSERT_TRUE(!torrent::sap_equal(sin_1_5000, sin_2_5000));
  ASSERT_TRUE(!torrent::sap_equal(sin6_1_5000, sin6_2_5000));
  ASSERT_TRUE(!torrent::sap_equal(sin_1_5000, sin_2_5100));
  ASSERT_TRUE(!torrent::sap_equal(sin6_1_5000, sin6_2_5100));
...
TEST_F(test_socket_address, test_sa_equal_addr) {
  ...
  ASSERT_TRUE(!torrent::sap_equal_addr(torrent::sa_make_unspec(),
                                       torrent::sa_make_inet6()));
  ASSERT_TRUE(!torrent::sap_equal_addr(torrent::sa_make_inet(),
                                       torrent::sa_make_inet6()));
  ASSERT_TRUE(!torrent::sap_equal_addr(torrent::sa_make_inet6(),
                                       torrent::sa_make_inet()));
  ...
  ASSERT_TRUE(!torrent::sap_equal_addr(sin_1, sin_2));
  ASSERT_TRUE(!torrent::sap_equal_addr(sin_1, c_sin_2));
  ASSERT_TRUE(!torrent::sap_equal_addr(c_sin_1, sin_2));
  ASSERT_TRUE(!torrent::sap_equal_addr(c_sin_1, c_sin_2));
  ...
  ASSERT_TRUE(!torrent::sap_equal_addr(sin6_1, sin6_2));
  ASSERT_TRUE(!torrent::sap_equal_addr(sin6_1, c_sin6_2));
  ASSERT_TRUE(!torrent::sap_equal_addr(c_sin6_1, sin6_2));
  ASSERT_TRUE(!torrent::sap_equal_addr(c_sin6_1, c_sin6_2));
  ...
  ASSERT_TRUE(!torrent::sap_equal_addr(sin_1_5000, sin_2_5000));
  ASSERT_TRUE(!torrent::sap_equal_addr(sin6_1_5000, sin6_2_5000));
  ASSERT_TRUE(!torrent::sap_equal_addr(sin_1_5000, sin_2_5100));
  ASSERT_TRUE(!torrent::sap_equal_addr(sin6_1_5000, sin6_2_5100));
...
TEST_F(test_socket_address, test_sa_copy) {
  ...
  ASSERT_TRUE(torrent::sap_copy(sin_1).get() != sin_1.get());
  ASSERT_TRUE(torrent::sap_copy(c_sin_1).get() != c_sin_1.get());
  ASSERT_TRUE(torrent::sap_copy(sin6_1).get() != sin6_1.get());
  ASSERT_TRUE(torrent::sap_copy(c_sin6_1).get() != c_sin6_1.get());
  ASSERT_TRUE(torrent::sap_copy(sin_1_5000).get() != sin_1_5000.get());
  ASSERT_TRUE(torrent::sap_copy(sin6_1_5000).get() != sin6_1_5000.get());
...
TEST_F(test_socket_address, test_sa_copy_addr) {
  ...
  ASSERT_TRUE(torrent::sap_copy_addr(sin_1).get() != sin_1.get());
  ASSERT_TRUE(torrent::sap_copy_addr(c_sin_1).get() != c_sin_1.get());
  ASSERT_TRUE(torrent::sap_copy_addr(sin6_1).get() != sin6_1.get());
  ASSERT_TRUE(torrent::sap_copy_addr(c_sin6_1).get() != c_sin6_1.get());
  ASSERT_TRUE(torrent::sap_copy_addr(sin_1_5000).get() != sin_1_5000.get());
  ASSERT_TRUE(torrent::sap_copy_addr(sin6_1_5000).get() != sin6_1_5000.get());
