...
TEST_F(test_address_info, test_helpers) {
  ...
  ASSERT_TRUE(sin_zero->sin_family == AF_INET);
  ASSERT_TRUE(sin_zero->sin_port == 0);
  ASSERT_TRUE(sin_zero->sin_addr.s_addr == in_addr().s_addr);

  torrent::sin_unique_ptr sin_1 =
    torrent::sin_from_sa(wrap_ai_get_first_sa("1.2.3.4"));
  ASSERT_TRUE(sin_1 != nullptr);
  ASSERT_TRUE(sin_1->sin_family == AF_INET);
  ASSERT_TRUE(sin_1->sin_port == 0);
  ASSERT_TRUE(sin_1->sin_addr.s_addr == htonl(0x01020304));

  torrent::sin6_unique_ptr sin6_zero =
    torrent::sin6_from_sa(wrap_ai_get_first_sa("::"));
  ASSERT_TRUE(sin6_zero != nullptr);
  ASSERT_TRUE(sin6_zero->sin6_family == AF_INET6);
  ASSERT_TRUE(sin6_zero->sin6_port == 0);
  ...
  ASSERT_TRUE(sin6_1 != nullptr);
  ASSERT_TRUE(sin6_1->sin6_family == AF_INET6);
  ASSERT_TRUE(sin6_1->sin6_port == 0);
