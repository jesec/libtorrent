...
dump_failure_log(const failure_type& failure) {
  ...
  std::for_each(failure.log->begin(),
                failure.log->end(),
...
dump_failures(const failure_list_type& failures) {
  ...
  std::for_each(failures.begin(),
                failures.end(),
                [](const failure_type& failure) { dump_failure_log(failure); });
  std::cout << std::endl;
}

static void
add_tests(CppUnit::TextUi::TestRunner& runner, const char* c_test_names) {
  ...
  while ((next = test_names.find(',', pos)) < test_names.size()) {
    runner.addTest(CppUnit::TestFactoryRegistry::getRegistry(
                     test_names.substr(pos, next - pos))
                     .makeTest());
    pos = next + 1;
  }

  runner.addTest(
    CppUnit::TestFactoryRegistry::getRegistry(test_names.substr(pos))
      .makeTest());
}
