#ifndef LIBTORRENT_TEST_HELPERS_MOCK_COMPARE_H
...
inline bool
mock_compare_arg(Arg lhs, Arg rhs) {
  return lhs == rhs;
}

template<int I, typename A, typename... Args>
typename std::enable_if<I == 1, int>::type
mock_compare_tuple(const std::tuple<A, Args...>& lhs,
...
struct mock_compare_map {
  ...
  static T* begin_pointer() {
    return reinterpret_cast<T*>(0x1000);
  }
  static T* end_pointer() {
    return reinterpret_cast<T*>(0x2000);
  }
  ...
  static bool has_value(const T* v) {
    return std::any_of(
      values.begin(), values.end(), [v](typename values_type::value_type& kv) {
        return v == kv.second;
      });
  }
...
template<>
inline bool
mock_compare_arg<sockaddr*>(sockaddr* lhs, sockaddr* rhs) {
  return lhs != nullptr && rhs != nullptr && torrent::sa_equal(lhs, rhs);
}
template<>
inline bool
mock_compare_arg<const sockaddr*>(const sockaddr* lhs, const sockaddr* rhs) {
  return lhs != nullptr && rhs != nullptr && torrent::sa_equal(lhs, rhs);
}

template<>
inline bool
mock_compare_arg<torrent::Event*>(torrent::Event* lhs, torrent::Event* rhs) {
  if (mock_compare_map<torrent::Event>::is_key(lhs)) {
    ...
    return mock_compare_map<torrent::Event>::has_key(lhs) &&
           mock_compare_map<torrent::Event>::get(lhs) == rhs;
  }
