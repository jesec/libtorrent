...
#define TEST_DEFAULT_SA                                                        \
  ...
  auto sin6_v4_bc_5000 =                                                       \
    wrap_ai_get_first_sa("::ffff:255.255.255.255", "5000");                    \
...
inline bool
compare_listen_result(const torrent::listen_result_type& lhs,
                      int                                rhs_fd,
                      const torrent::c_sa_unique_ptr&    rhs_sap) {
  return lhs.fd == rhs_fd && ((lhs.address && rhs_sap) ||
                              ((lhs.address && rhs_sap) &&
                               torrent::sap_equal(lhs.address, rhs_sap)));
}

inline torrent::sa_unique_ptr
wrap_ai_get_first_sa(const char*     nodename,
...
wrap_ai_get_first_c_sa(const char*     nodename,
                       const char*     servname = nullptr,
                       const addrinfo* hints    = nullptr) {
  auto sa = torrent::ai_get_first_sa(nodename, servname, hints);
...
constexpr ai_flags_enum
operator|(ai_flags_enum a, ai_flags_enum b) {
...
test_valid_ai_ref(const test_ai_ref& ftor, uint16_t port = 0) {
  ...
  if (int err = ftor(ai)) {
    std::cout << std::endl
              << "valid_ai_ref got error '" << gai_strerror(err) << "'"
              << std::endl;
    return false;
  }
...
test_valid_ai_ref_err(const test_ai_ref& ftor, int expect_err) {
  ...
  if (err != expect_err) {
    std::cout << std::endl
              << "ai_ref_err got wrong error, expected '"
              << gai_strerror(expect_err) << "', got '" << gai_strerror(err)
              << "'" << std::endl;
    return false;
  }
