#ifndef LIBTORRENT_TEST_HELPERS_MOCK_FUNCTION_H
#define LIBTORRENT_TEST_HELPERS_MOCK_FUNCTION_H
...
template<typename R, typename... Args>
typename mock_function_map<R, Args...>::func_map_type
  mock_function_map<R, Args...>::functions;
template<typename R, typename... Args>
typename mock_function_map<R, Args...>::redirect_map_type
  mock_function_map<R, Args...>::redirects;
...
mock_cleanup_map(R fn [[gnu::unused]] (Args...)) {
  return mock_function_type<R, Args...>::type::cleanup(
...
mock_expect(R fn(Args...), R ret, Args... args) {
  ...
  mock_map::functions[reinterpret_cast<void*>(fn)].push_back(
    std::tuple<R, Args...>(ret, args...));
}
...
mock_expect(void fn(Args...), Args... args) {
  ...
  mock_map::functions[reinterpret_cast<void*>(fn)].push_back(
    std::tuple<mock_void, Args...>(mock_void(), args...));
}
...
template<typename R, typename... Args>
auto
mock_call(std::string name, R fn(Args...), Args... args)
  -> decltype(fn(args...)) {
  typedef mock_function_type<R, Args...> mock_type;
