...
public:
  ...
  DiffieHellman(const unsigned char prime[],
                int                 primeLength,
  ...
  std::string secret_str() const {
    return std::string(m_secret.get(), m_size);
  }

private:
  DiffieHellman(const DiffieHellman& dh) = delete;
  DiffieHellman& operator=(const DiffieHellman& dh) = delete;
