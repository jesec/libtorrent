...
public:
  ...
  const TrackerDht* tracker() const {
    return m_tracker;
  }
...
public:
  // transaction packet
  DhtTransactionPacket(const utils::socket_address* s,
                       const DhtMessage&            d,
                       unsigned int                 id,
                       DhtTransaction*              t)
    : m_sa(*s)
  ...
  int age() const {
    return has_transaction() ? 0 : cachedTime.seconds() + m_id;
  }
  const DhtTransaction* transaction() const {
    return m_transaction;
  }
  DhtTransaction* transaction() {
    return m_transaction;
  }
...
public:
  ...
  static key_type key(const utils::socket_address* sa, int id);
  static bool     key_match(key_type key, const utils::socket_address* sa);

  // Node ID and address.
  const HashString& id() {
    return m_id;
  }
  const utils::socket_address* address() {
    return &m_sa;
  }
  ...
  int quick_timeout() {
    return m_quickTimeout;
  }
  bool has_quick_timeout() {
...
protected:
  DhtTransaction(int                          quick_timeout,
                 int                          timeout,
                 const HashString&            id,
...
protected:
  DhtTransactionSearch(int                        quick_timeout,
                       int                        timeout,
                       DhtSearch::const_accessor& node)
    : DhtTransaction(quick_timeout,
                     timeout,
                     node.node()->id(),
                     node.node()->address())
    , m_node(node)
...
public:
  ...
  const HashString& info_hash() {
    return m_infoHash;
  }
  raw_string info_hash_raw_string() const {
    return raw_string(m_infoHash.data(), HashString::size_data);
  }
  raw_string token() {
    return m_token;
  }
...
DhtTransaction::key(const utils::socket_address* sa, int id) {
  return ((uint64_t)sa->sa_inet()->address_n() << 32) + id;
}

inline bool
DhtTransaction::key_match(key_type key, const utils::socket_address* sa) {
  return (key >> 32) == sa->sa_inet()->address_n();
}

// These could (should?) check that the type matches, or use dynamic_cast if we
// have RTTI.
