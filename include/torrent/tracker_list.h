...
public:
  ...
  void     insert_url(unsigned int       group,
                      const std::string& url,
                      bool               extra_tracker = false);
  ...
  iterator find(Tracker* tb) {
    return std::find(begin(), end(), tb);
  }
  ...
  iterator       end_group(unsigned int group) {
    return begin_group(group + 1);
  }
  const_iterator end_group(unsigned int group) const {
    return begin_group(group + 1);
  }
  ...
  slot_string& slot_failure() {
    return m_slot_failed;
  }

  slot_tracker& slot_scrape_success() {
    return m_slot_scrape_success;
  }
  slot_string& slot_scrape_failure() {
    return m_slot_scrape_failed;
  }
