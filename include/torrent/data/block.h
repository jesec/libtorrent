...
public:
  ...
  size_type size_all() const {
    return m_queued.size() + m_transfers.size();
  }
  size_type size_not_stalled() const {
    return m_notStalled;
  }
  ...
  const BlockList* parent() const {
    return m_parent;
  }
  void set_parent(BlockList* p) {
    m_parent = p;
  }
  ...
  const transfer_list_type* queued() const {
    return &m_queued;
  }
  const transfer_list_type* transfers() const {
    return &m_transfers;
  }

  // The leading transfer, whom's data we're currently using.
  BlockTransfer* leader() {
    return m_leader;
  }
  const BlockTransfer* leader() const {
    return m_leader;
  }
  ...
  BlockFailed* failed_list() {
    return m_failedList;
  }
  void set_failed_list(BlockFailed* f) {
    m_failedList = f;
  }

  static void create_dummy(BlockTransfer* transfer,
                           PeerInfo*      peerInfo,
                           const Piece&   piece);
