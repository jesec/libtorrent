...
public:
  ...
  bool is_done() const {
    return completed_chunks() == size_chunks();
  }
  ...
  uint32_t size_chunks() const {
    return bitfield()->size_bits();
  }

  uint32_t completed_chunks() const {
    return bitfield()->size_set();
  }
  ...
  const Bitfield* bitfield() const {
    return m_data.completed_bitfield();
  }
  ...
  iterator merge(iterator_range range, const Path& path) {
    return merge(range.first, range.second, path);
  }
...
protected:
  ...
  iterator inc_completed(iterator firstItr,
                         uint32_t index) LIBTORRENT_NO_EXPORT;
...
private:
  bool        open_file(File*       node,
                        const Path& lastPath,
                        int         flags) LIBTORRENT_NO_EXPORT;
  void        make_directory(Path::const_iterator pathBegin,
                             Path::const_iterator pathEnd,
                             Path::const_iterator startItr) LIBTORRENT_NO_EXPORT;
  MemoryChunk create_chunk_part(FileList::iterator itr,
                                uint64_t           offset,
                                uint32_t           length,
                                int                prot) LIBTORRENT_NO_EXPORT;
...
file_list_contains_position(FileList* file_list, uint64_t pos) {
  return std::find_if(file_list->begin(), file_list->end(), [pos](File* f) {
    return f->is_valid_position(pos);
  });
}
