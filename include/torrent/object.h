...
public:
  ...
  static constexpr uint32_t flag_unordered =
    0x100; // bencode dictionary was not sorted
  static constexpr uint32_t flag_static_data =
    0x010000; // Object does not change across sessions.
  static constexpr uint32_t flag_session_data =
    0x020000; // Object changes between sessions.
  static constexpr uint32_t flag_function = 0x040000; // A function object.
  static constexpr uint32_t flag_function_q1 =
    0x080000; // A quoted function object.
  static constexpr uint32_t flag_function_q2 =
    0x100000; // A double-quoted function object.

  static constexpr uint32_t mask_function =
    0x1C0000; // Mask for function objects.
  ...
  Object(const value_type v)
    : m_flags(TYPE_VALUE) {
    new (&_value()) value_type(v);
  }
  Object(const char* s)
    : m_flags(TYPE_STRING) {
    new (&_string()) string_type(s);
  }
  Object(const string_type& s)
    : m_flags(TYPE_STRING) {
    new (&_string()) string_type(s);
  }
  Object(const raw_bencode& r)
    : m_flags(TYPE_RAW_BENCODE) {
    new (&_raw_bencode()) raw_bencode(r);
  }
  Object(const raw_string& r)
    : m_flags(TYPE_RAW_STRING) {
    new (&_raw_string()) raw_string(r);
  }
  Object(const raw_list& r)
    : m_flags(TYPE_RAW_LIST) {
    new (&_raw_list()) raw_list(r);
  }
  Object(const raw_map& r)
    : m_flags(TYPE_RAW_MAP) {
    new (&_raw_map()) raw_map(r);
  }
  ...
  static Object create_value() {
    return Object(value_type());
  }
  static Object create_string() {
    return Object(string_type());
  }
  static Object create_list() {
    Object tmp;
    tmp.m_flags = TYPE_LIST;
    new (&tmp._list()) list_type();
    return tmp;
  }
  static Object create_map() {
    Object tmp;
    tmp.m_flags    = TYPE_MAP;
    tmp._map_ptr() = new map_type();
    return tmp;
  }
  ...
  type_type type() const {
    return (type_type)(m_flags & mask_type);
  }
  uint32_t flags() const {
    return m_flags & mask_flags;
  }

  void set_flags(uint32_t f) {
    m_flags |= f & mask_public;
  }
  void unset_flags(uint32_t f) {
    m_flags &= ~(f & mask_public);
  }

  void set_internal_flags(uint32_t f) {
    m_flags |= f & (mask_internal & ~mask_type);
  }
  ...
  bool is_dict_key() const {
    return type() == TYPE_DICT_KEY;
  }
  bool is_raw_bencode() const {
    return type() == TYPE_RAW_BENCODE;
  }
  bool is_raw_string() const {
    return type() == TYPE_RAW_STRING;
  }
  bool is_raw_list() const {
    return type() == TYPE_RAW_LIST;
  }
  bool is_raw_map() const {
    return type() == TYPE_RAW_MAP;
  }

  value_type& as_value() {
    check_throw(TYPE_VALUE);
    return _value();
  }
  ...
  const string_type& as_string_c() const {
    check_throw(TYPE_STRING);
    return _string();
  }
  ...
  string_type& as_dict_key() {
    check_throw(TYPE_DICT_KEY);
    return _dict_key().first;
  }
  ...
  template<typename T>
  value_type& get_key_value(const T& k) {
    return get_key(k).as_value();
  }
  template<typename T>
  const value_type& get_key_value(const T& k) const {
    return get_key(k).as_value();
  }
  template<typename T>
  string_type& get_key_string(const T& k) {
    return get_key(k).as_string();
  }
  template<typename T>
  const string_type& get_key_string(const T& k) const {
    return get_key(k).as_string();
  }
  template<typename T>
  list_type& get_key_list(const T& k) {
    return get_key(k).as_list();
  }
  template<typename T>
  const list_type& get_key_list(const T& k) const {
    return get_key(k).as_list();
  }
  template<typename T>
  map_type& get_key_map(const T& k) {
    return get_key(k).as_map();
  }
  template<typename T>
  const map_type& get_key_map(const T& k) const {
    return get_key(k).as_map();
  }

  Object& insert_key(const key_type& k, const Object& b) {
    check_throw(TYPE_MAP);
    return _map()[k] = b;
  }
  Object& insert_key_move(const key_type& k, Object& b) {
    check_throw(TYPE_MAP);
    return _map()[k].move(b);
  }
  ...
  // something similar.
  map_insert_type insert_preserve_any(const key_type& k, const Object& b) {
    check_throw(TYPE_MAP);
    return _map().insert(map_type::value_type(k, b));
  }
  map_insert_type insert_preserve_type(const key_type& k, Object& b);
  map_insert_type insert_preserve_copy(const key_type& k, Object b) {
    return insert_preserve_type(k, b);
  }
  ...
  Object& insert_front(const Object& b) {
    check_throw(TYPE_LIST);
    return *_list().insert(_list().begin(), b);
  }
  Object& insert_back(const Object& b) {
    check_throw(TYPE_LIST);
    return *_list().insert(_list().end(), b);
  }
...
private:
  ...
  template<typename T>
  void check_value_throw(const char* err_msg) const;
...
inline Object::Object(const Object& b) {
  ...
  switch (type()) {
    ...
    case TYPE_VALUE:
      ...
      break;
    case TYPE_STRING:
      new (&_string()) string_type(b._string());
      break;
    case TYPE_LIST:
      new (&_list()) list_type(b._list());
      break;
    case TYPE_MAP:
      _map_ptr() = new map_type(b._map());
      break;
    case TYPE_DICT_KEY:
      new (&_dict_key().first) string_type(b._dict_key().first);
      _dict_key().second = new Object(*b._dict_key().second);
      break;
  }
}
...
Object::create_empty(type_type t) {
  switch (t) {
    case TYPE_RAW_BENCODE:
      return create_raw_bencode();
    case TYPE_RAW_STRING:
      return create_raw_string();
    case TYPE_RAW_LIST:
      return create_raw_list();
    case TYPE_RAW_MAP:
      return create_raw_map();
    case TYPE_VALUE:
      return create_value();
    case TYPE_STRING:
...
Object
object_create_normal(const raw_bencode& obj) LIBTORRENT_EXPORT;
Object
object_create_normal(const raw_list& obj) LIBTORRENT_EXPORT;
Object
object_create_normal(const raw_map& obj) LIBTORRENT_EXPORT;
inline Object
object_create_normal(const raw_string& obj) {
  return torrent::Object(obj.as_string());
}
...
Object::create_raw_bencode(raw_bencode obj) {
  Object tmp;
  tmp.m_flags = TYPE_RAW_BENCODE;
  new (&tmp._raw_bencode()) raw_bencode(obj);
  return tmp;
}
...
Object::create_raw_string(raw_string obj) {
  Object tmp;
  tmp.m_flags = TYPE_RAW_STRING;
  new (&tmp._raw_string()) raw_string(obj);
  return tmp;
}
...
Object::create_raw_list(raw_list obj) {
  Object tmp;
  tmp.m_flags = TYPE_RAW_LIST;
  new (&tmp._raw_list()) raw_list(obj);
  return tmp;
}
...
Object::create_raw_map(raw_map obj) {
  Object tmp;
  tmp.m_flags = TYPE_RAW_MAP;
  new (&tmp._raw_map()) raw_map(obj);
  return tmp;
}
...
object_create_normal(const Object& obj) {
  switch (obj.type()) {
    case Object::TYPE_RAW_BENCODE:
      return object_create_normal(obj.as_raw_bencode());
    case Object::TYPE_RAW_STRING:
      return object_create_normal(obj.as_raw_string());
    case Object::TYPE_RAW_LIST:
      return object_create_normal(obj.as_raw_list());
    case Object::TYPE_RAW_MAP:
      return object_create_normal(obj.as_raw_map());
    default:
      return obj;
  }
}
...
object_create_string(const torrent::Object& obj) {
  switch (obj.type()) {
    case Object::TYPE_RAW_BENCODE:
      return obj.as_raw_bencode().as_raw_string().as_string();
    case Object::TYPE_RAW_STRING:
      return obj.as_raw_string().as_string();
    default:
...
Object::create_list_range(ForwardIterator first, ForwardIterator last) {
  Object tmp;
  tmp.m_flags = TYPE_LIST;
  new (&tmp._list()) list_type(first, last);
  return tmp;
}
...
Object::clear() {
  switch (type()) {
    ...
    case TYPE_MAP:
      ...
      break;
    case TYPE_DICT_KEY:
      delete _dict_key().second;
      _dict_key().~dict_key_type();
      break;
    default:
      break;
  }
...
Object::swap_same_type(Object& left, Object& right) {
  ...
  switch (left.type()) {
    ...
    case Object::TYPE_LIST:
      left._list().swap(right._list());
      break;
    case Object::TYPE_DICT_KEY:
      std::swap(left._dict_key().first, right._dict_key().first);
      std::swap(left._dict_key().second, right._dict_key().second);
      break;
    default:
      std::swap(left.t_pod, right.t_pod);
      break;
  }
}
...
object_equal(const Object& left, const Object& right) {
  ...
  switch (left.type()) {
    ...
    case Object::TYPE_VALUE:
      return left.as_value() == right.as_value();
    case Object::TYPE_STRING:
...
Object::check_value_throw(const char* err_msg) const {
  ...
  if (!(_value() >= std::numeric_limits<T>::min() &&
