...
#define RAW_BENCODE_SET_USING                                                  \
  ...
  bool operator==(const this_type& rhs) const {                                \
    return raw_object::operator==(rhs);                                        \
  }                                                                            \
  bool operator!=(const this_type& rhs) const {                                \
    return raw_object::operator!=(rhs);                                        \
  }
...
public:
  ...
  raw_bencode()
    : raw_object() {}
  raw_bencode(value_type* src_data, size_type src_size)
  ...
  bool is_value() const {
    return m_size >= 3 && m_data[0] >= 'i';
  }
  bool is_raw_string() const {
    return m_size >= 2 && m_data[0] >= '0' && m_data[0] <= '9';
  }
  bool is_raw_list() const {
    return m_size >= 2 && m_data[0] >= 'l';
  }
  bool is_raw_map() const {
    return m_size >= 2 && m_data[0] >= 'd';
  }
  ...
  static raw_bencode from_c_str(const char* str) {
    return raw_bencode(str, std::strlen(str));
  }
};
...
public:
  ...
  static raw_string from_c_str(const char* str) {
    return raw_string(str, std::strlen(str));
  }
  static raw_string from_string(const std::string& str) {
    return raw_string(str.data(), str.size());
  }
};
...
public:
  ...
  static raw_list from_c_str(const char* str) {
    return raw_list(str, std::strlen(str));
  }
};
...
raw_bencode_equal(const raw_bencode& left, const raw_bencode& right) {
  return left.size() == right.size() &&
         std::memcmp(left.begin(), right.begin(), left.size()) == 0;
}

template<typename tmpl_raw_object>
inline bool
raw_bencode_equal(const tmpl_raw_object& left,
                  const char*            right,
                  size_t                 right_size) {
  return left.size() == right_size &&
         std::memcmp(left.begin(), right, right_size) == 0;
}
