...
struct static_map_mapping_type {
  ...
  static bool is_not_key_char(char c) {
    return c == '\0' || c == ':' || c == '[' || c == '*';
  }

  const char* find_key_end(const char* pos) const {
    return std::find_if(pos, key + max_key_size, &is_not_key_char);
  }
...
public:
  ...
  const entry_type* values() const {
    return m_values;
  }

  Object& operator[](key_type key) {
    return m_values[key].object;
  }
  const Object& operator[](key_type key) const {
    return m_values[key].object;
  }
...
struct static_map_stack_type {
  void set_key_index(uint32_t                   start_index,
                     uint32_t                   end_index,
                     uint32_t                   delim_size,
...
using static_map_key_search_result =
  std::pair<const static_map_mapping_type*, unsigned int>;
...
find_key_match(const static_map_mapping_type* first,
               ...
               const char*                    key_last) LIBTORRENT_EXPORT;
