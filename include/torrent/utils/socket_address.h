...
public:
  static constexpr sa_family_t af_inet   = AF_INET;
  static constexpr int         pf_inet   = PF_INET;
  static constexpr sa_family_t af_inet6  = AF_INET6;
  static constexpr int         pf_inet6  = PF_INET6;
  static constexpr sa_family_t af_unspec = AF_UNSPEC;
  static constexpr int         pf_unspec = PF_UNSPEC;

#ifdef AF_LOCAL
  static constexpr sa_family_t af_local = AF_LOCAL;
  static constexpr int         pf_local = PF_LOCAL;
#else
  static constexpr sa_family_t af_local = AF_UNIX;
  static constexpr int         pf_local = PF_UNIX;
#endif
  ...
  bool is_valid_inet_class() const {
  ...
  bool set_address_str(const std::string& a) {
    return set_address_c_str(a.c_str());
  }
  bool set_address_c_str(const char* a);

  uint32_t length() const;

  socket_address_inet* sa_inet() {
    return reinterpret_cast<socket_address_inet*>(this);
  }
  const socket_address_inet* sa_inet() const {
    return reinterpret_cast<const socket_address_inet*>(this);
  }

  sockaddr* c_sockaddr() {
    return &m_sockaddr;
  }
  sockaddr_in* c_sockaddr_inet() {
    return &m_sockaddrInet;
  }

  const sockaddr* c_sockaddr() const {
    return &m_sockaddr;
  }
  const sockaddr_in* c_sockaddr_inet() const {
    return &m_sockaddrInet;
  }

  socket_address_inet6* sa_inet6() {
    return reinterpret_cast<socket_address_inet6*>(this);
  }
  const socket_address_inet6* sa_inet6() const {
    return reinterpret_cast<const socket_address_inet6*>(this);
  }

  sockaddr_in6* c_sockaddr_inet6() {
    return &m_sockaddrInet6;
  }
  ...
  static socket_address* cast_from(sockaddr* sa) {
    return reinterpret_cast<socket_address*>(sa);
  }
  static const socket_address* cast_from(const sockaddr* sa) {
    return reinterpret_cast<const socket_address*>(sa);
  }
...
public:
  bool is_any() const {
    return is_port_any() && is_address_any();
  }
  bool is_valid() const {
    return !is_port_any() && !is_address_any();
  }
  bool is_port_any() const {
    return port() == 0;
  }
  bool is_address_any() const {
    return m_sockaddr.sin_addr.s_addr == htonl(INADDR_ANY);
  }

  void clear() {
    std::memset(this, 0, sizeof(socket_address_inet));
    set_family();
  }
  ...
  uint16_t port_n() const {
    return m_sockaddr.sin_port;
  }
  void set_port(uint16_t p) {
    m_sockaddr.sin_port = htons(p);
  }
  void set_port_n(uint16_t p) {
    m_sockaddr.sin_port = p;
  }
  ...
  uint32_t address_h() const {
    return ntohl(m_sockaddr.sin_addr.s_addr);
  }
  uint32_t address_n() const {
    return m_sockaddr.sin_addr.s_addr;
  }
  std::string address_str() const;
  bool        address_c_str(char* buf, socklen_t size) const;

  void set_address(in_addr a) {
    m_sockaddr.sin_addr = a;
  }
  void set_address_h(uint32_t a) {
    m_sockaddr.sin_addr.s_addr = htonl(a);
  }
  void set_address_n(uint32_t a) {
    m_sockaddr.sin_addr.s_addr = a;
  }
  bool set_address_str(const std::string& a) {
    return set_address_c_str(a.c_str());
  }
  ...
  void set_address_any() {
    set_port(0);
    set_address_h(INADDR_ANY);
  }

  sa_family_t family() const {
    return m_sockaddr.sin_family;
  }
  void set_family() {
...
public:
  ...
  bool is_address_any() const {
    return std::memcmp(&m_sockaddr.sin6_addr, &in6addr_any, sizeof(in6_addr)) ==
           0;
  }

  void clear() {
    std::memset(this, 0, sizeof(socket_address_inet6));
    set_family();
  }
  ...
  uint16_t port_n() const {
    return m_sockaddr.sin6_port;
  }
  void set_port(uint16_t p) {
    m_sockaddr.sin6_port = htons(p);
  }
  void set_port_n(uint16_t p) {
    m_sockaddr.sin6_port = p;
  }

  in6_addr address() const {
    return m_sockaddr.sin6_addr;
  }
  const in6_addr* address_ptr() const {
    return &m_sockaddr.sin6_addr;
  }
  std::string address_str() const;
  bool        address_c_str(char* buf, socklen_t size) const;

  void set_address(in6_addr a) {
    m_sockaddr.sin6_addr = a;
  }
  bool set_address_str(const std::string& a) {
    return set_address_c_str(a.c_str());
  }
  ...
  void set_address_any() {
    set_port(0);
    set_address(in6addr_any);
  }

  std::string pretty_address_str() const;

  sa_family_t family() const {
    return m_sockaddr.sin6_family;
  }
  void set_family() {
...
socket_address::copy_sockaddr(const sockaddr* src) {
  std::memset(this, 0, sizeof(socket_address));
  std::memcpy(
    static_cast<void*>(this), src, socket_address::cast_from(src)->length());
}
  ...
  switch (family()) {
    ...
    default:
      throw std::logic_error(
        "socket_address::operator == (rhs) invalid type comparison.");
  }
}
...
socket_address_inet::operator==(const socket_address_inet& rhs) const {
  return m_sockaddr.sin_addr.s_addr == rhs.m_sockaddr.sin_addr.s_addr &&
...
socket_address_inet6::operator==(const socket_address_inet6& rhs) const {
  return memcmp(&m_sockaddr.sin6_addr,
                &rhs.m_sockaddr.sin6_addr,
                sizeof(in6_addr)) == 0 &&
...
socket_address_inet6::operator<(const socket_address_inet6& rhs) const {
  int addr_comp =
    memcmp(&m_sockaddr.sin6_addr, &rhs.m_sockaddr.sin6_addr, sizeof(in6_addr));
  return addr_comp < 0 ||
